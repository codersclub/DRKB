...
* Имена операторов и курсоров
    - statement-id (изготовленный оператор)
    - cursor-name (курсор)
                                      
* Объекты экранного обмена
    - window
    - form
    - screen-field
    - screen-record
...
Поэтому i\_Un103Tt и I\_UN103TT - одно и тоже имя.
...
где type - тип простой переменной, конструкция RECORD, или конструкция ARRAY.
...
Для описания переменных служит оператор DEFINE:
    ...
    # Переменную можно объявить с ключевым словом LIKE column\_name.
    ...
    #  LIKE. Здесь объявлена запись rrr, содержащая элементы,
    #  имеющие те же самые названия и те же самые типы что и столбцы
    #  таблицы kadry
...
программы.
    ...
    #    Если функция возвращает одно значение,
    #    то ее имя можно использовать в выражениях.
      MESSAGE simw
    . . .
    END MAIN
...
Условный оператор.
    ...
    IF str = "завершить" OR y<0 THEN exit program
      # Не забывайте в конце каждого условного # оператора ставить END IF.
    END IF
...
попросят ввести оператор.
    ...
    . . .
      LABEL METK2:PROMPT "введите оператор языка SQL: " FOR stroka
       ...
       IF status\<0 THEN ERROR "ошибка номер ", status, " в вашем операторе" GOTO metk2 END IF
       WHENEVER ERROR STOP # Восстановить режим
       # "В случае ошибки прервать # выполнение программы"
    . . .
    END MAIN
...
С помощью конструкции циклической `FOREACH имя_курсора ... END FOREACH`
можно перебрать все строки, выбранные оператором SELECT.
Оператор OPEN в этом случае не нужен.
...
curs2 помещаются в массив z1 (но не более 100 строк).
    ...
    FOR counter="1" TO 100
      FETCH curs2 INTO z1[counter].* # взять очередную строку
                                     # и поместить ее в следующий элемент массива z1
      IF status="NOTFOUND" THEN # если выбранные сроки кончились, закончить цикл
        EXIT
    FOR END
    IF END
      FOR LET counter="counter-1"
...
аналогичны предыдущему примеру.
    ...
    PROMPT "допишите оператор SELECT * FROM ceh WHERE "
    FOR simw
