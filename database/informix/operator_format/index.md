...
Логические выражения:
    ...
    спецсимволы шаблона % _  означают "0 или несколько символов", "один любой символ" !
...
Системные переменные (Устанавливаются после любого оператора 4GL):
    ...
    SQLERRD[2] serial значение или ISAM error code
...
Программные операторы.
    ...
    RUN {"командная строка UNIX"|char-variable}
        [RETURNING int-variable | WITHOUT WAITING]
...
Меню, окна.
    ...
    MENU  "Название меню"
        COMMAND { KEY (key-list) | [KEY (key-list)] "kоманда меню"
...
Простые операторы вывода на экран.
    ...
    PROMPT список переменных и констант
...
EXECUTE statment-id [USING input-list] - Выполняет, заменив знаки "?" на input-list
...
**Внимание:**  
SCROLL cursor нельзя открывать FOR UPDATE,  зато для не-SCROLL
cursora можно использовать

**Внимание:**  
оператор DECLARE cursor-name должен располагаться в тексте
...
Операторы транзакции и восстановления:
...
где qualifier in {YEAR,MONTH,DAY,HOUR,MINUTE,SECOND,FRACTION(n)}
    ...
    SET EXPLAIN {ON | OFF} - Выводить системные объяснения в sqlexplain.out
...
Операторы манипуляции данными.
    ...
    [,...]
    | {(col-list) | [table.]*} = { ({ expr-list | (SELECT-st) } [,...] | record-name.* }
...
где conditions - связанные логическими операторами OR, AND, NOT сравнения
...
Операторы задания прав доступа (не откатываются).
    ...
    SET LOCK MODE TO [NOT] WAIT
         ждать [не ждать] освобождения блокир. строк
...
**Примечание:**
подчеркнутые операторы нельзя использовать в 4GL, а можно
только в INFORMIX-SQL

    INFO { TABLES | { COLUMNS | INDEXES | ACCES | PRIVILEGES | STATUS } 
         FOR table-name }
