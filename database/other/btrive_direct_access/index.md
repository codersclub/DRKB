...
Данный документ - краткая инструкция по использованию в DELPHI
...
Прежде всего в директорию, содержащую исходники
...
if `status = B_NO_ERROR` значит подконнектилось успешно, иначе - по
...
Для работы с конкретной таблицей необходимо: Чтобы на эту таблицу было наложено заклинание эксклюзивного
доступа... (Sorry, пиво...)

Продолжаю на трезвую голову. Так вот: для
...
Теперь - собственно описание синтаксиса. Для того, чтобы
выполнить B\_GET\_EQUAL, нужно:

1. Описать структуру индексов. Например, в уже упомянутой таблице
...
Если `status <> B_NO_ERROR and <> B_KEY_VALUE_NOT_FOUND` -
...
Самое главное - здесь рассмотрен вариант
...
Итак: Непосредственно перед действием обновления у нас должен быть
...
Использование практически совпадает с
...
Вот, собственно и все, что мне известно. В принципе, в
