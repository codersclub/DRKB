...
Помните что если вы не учли какой-то из этих моментов НИКАКИХ ошибок
...
локальные базы от MS и с серверами не работает даже теоретически.

Итого: из глобальных систем доступа к базам данных мы имеем ADO (OLE DB) и
...
надо думать нужно ли блокировать и если нужно то что именно.
...
это может привести к тому что данные будут неправильными в следствии
...
Следовательно, надо к началу блокировки иметь все нужные данные,
...
Теперь допустим Вася получил погоду в виде таблицы гигабайтного
...
много... А Вам нужны данные сейчас а не через пол часа. Что делать?

А делать надо не вам а Васе... И сделать ему надо такую простую штуку как
...
будет намертво заблокирована. Правда это приведёт к тому, что Васины действия по
...
Без индексов на этом поле сервер вынужден будет в цикле перебрать все
