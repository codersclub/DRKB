...
правила нормализации, каждая таблица должна описывать
один вид сущностей - человека, место, событие или вещь.
По этой причине, когда нужно сравнить информацию,
относящуюся к различным объектам, необходима
...
необходимо выполнить следующий запрос:
    ...
    au_id        au_lname  au_fname  phone         address          city      state postalcode  contract  pub_id  pub_name              city      state
    ------------ --------- --------- ------------- ---------------- --------- ----- ----------- --------- ------- --------------------- --------- -----
    238-95-7766  Carson    Cheryl    415 548-7723  589 Darwin Ln.   Berkeley  CA    94705               1 1389    Algodata Infosystems  Berkeley  CA
    409-56-7008  Bennet    Abraham   415 658-9932  223 Bateman St   Berkeley  CA    94705               1 1389    Algodata Infosystems  Berkeley  CA
...
- Названия базовых таблиц, на которые ссылаются вьюверы, указанные в
  предложении from.
...
чем". Детальное описание синтаксиса предложения where приводится в главе 2
этого руководства и в главе "Предложение where" в Справочном
руководстве SQL Сервера.

> **Замечание.**
> Можно получить совершенно неожиданный результат, если
> опустить предложение where в операторе соединения. Без этого предложения
> все вышеприведенные запросы на соединение будут выдавать 27 строк вместо 2.
> В следующем разделе будет объяснено почему так происходит.
...
внешних операций соединения относятся следующие операции.
    ...
    *=         В результат включаются все строки из первой таблицы,
               а не только строки, удовлетворящие условию сравнения.
...
> **Замечание.**
> Таблицы нельзя соединять по текстовым или графическим полям.
...
образом:
    ...
    where au1.city = "Oakland" and
          au2.city = "Oakland" and
          au1.state = "CA" and
          au2.state = "CA" and
          au1.postalcode = au2.postalcode
...
условие:
    ...
    where au1.city = "Oakland" and
          au2.city = "Oakland" and
          au1.state = "CA"
          and au2.state = "CA" and
          au1.postalcode = au2.postalcode and
          au1.au_id < au2.au_id
...
> **Замечание.**  
> Выражение "not название\_столбца1 = название\_столбца2"
...
> **Замечание.**  
> Поскольку столбцы с данными типа "бит" не допускают
