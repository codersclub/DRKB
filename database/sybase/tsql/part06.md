...
Категории типов данных	|Синонимы	|Диапазон	|Число байтов
-----------------|-----------------|----------------------|-----------------
**Точные числовые: целые** |
tinyint		|	|от 0 до 255	|1
smallint	|	|от 2^15^-1 (32,767) до -2^15^ (-32,768)	|2
int		|integer	|от 2^31^-1 (2,147,483,647) до -2^31^ (-2,147,483,648)	|4
**Точные числовые: десятичные** | | |
numeric (p,s)	|	|от 10^38^-1 до -10^38^	|от 2 до 17
decimal (p,s)	|dec	|от 10^38^-1 до -10^38^	|от 2 до 17
**Приближенные числовые** | | |
float precision	|	|зависит от компьютера	|4 или 8
double precision|	|зависит от компьютера	|8
real		|	|зависит от компьютера	|4
**Денежные** | | |
smallmoney	|	|от 214,748.3647 до-214,748.3648	|4
money		|	|от 922,337,203,685,477.5807 до - 922,337,203,685,477.5808	|8
**Дата/время** | | |
smalldatetime	|	|с 1 Января 1990 по 6 Июня 2079	|4
datetime	|	|с 1 Января 1753 по 31 Декабря 9999	|8
**Символьные** | | |
char(n)    | character | 255 символов или меньше | n
varchar(n) |character varying |255 символов или меньше | длина введенной цепочки char varying
nchar(n)   | national char, national character | 255 символов или меньше | n\*@@ncharsize
nvarchar(n) |n char varying, national char varying, national character varying | 255 символов или меньше  | @@ncharsize * число символов
text        |        | 2^31^-1 (2,147,483,647)  | 0 или кратно 2K байтов или меньше
**Бинарные** | | |
binary   | | 255 байтов или меньше  | n
varbinary | | 255 байтов или меньше | входная длина
image     | | 2^31-^1(2,147,483,647)  | 0 или кратно 2K байтов или меньше
**Битовые** | | |
bit     | | 0 или 1 | 1 (один байт содержит до 8 битов)
...
Тип данных |Диапазон сохраняемых чисел |Число байтов памяти
-----------|----------------------- |-----------------------
tinyint    |Все числа от 0 до 255, включительно.<br/>(Отрицательные значения не допускаются.)| 1
smallint   |Все числа от 2^15^-1 до -2^15^<br/>(32,767 до -32,768), включительно. | 2
int        |Все числа от 2^31^-1 до -2^31^<br/>(2,147,483,647 до -2,147,483,648), включительно | 4
...
-------------------------------|-----------------------------------
float[(точность_по_умолчанию)] | 4, если точность < 16,<br/>8, если точность >= 16
double precision               | 8
real                           | 4
...
- Строки в столбцах фиксированной длины с конца дополняются пробелями до длины этого столбца.
  Для данных типа char отводится n байтов памяти;
  а для данных типа nchar число n умножается на значение переменной @@ncharsize,
  которое равно средней длине (в байтах) символа национального алфавита.
  Когда создается столбец типа char и nchar, в котором допускаются неопределенные значения,
  то SQL Сервер автоматически преобразует их в типы varchar и nvarchar соответственно и сохраняет значения в соответствии с этими типами данных. (Этого не происходит для переменных типа char и nchar, и параметров).
...
> **Замечание:**  
> Обработка SQL Сервером двоичных
...
-----------|----------------------------|----------------------------------
money      | Денежные величины между +922,337,203,685,477.5807 и -922,337,203,685,477.5808 |    8
smallmoney | Денежные величины между +214,748.3647 и -214,748.3648 |     4
...
нескольких таблицах.
Более полная информация об этом будет дана в главе 12.
...
Процедура sp\_addtype вызывается следующим образом:
    ...
    sp_addtype название_типа_данных,
               системный_тип_данных  [(длина) | (точность [, шкала]]
...
и нестандартном типе данных tid:
    ...
    Type_name  Storage_type  Length  Prec    Scale  Nulls  Default_name  Rule_name  Identity
    ---------  ------------  ------  ------  -----  -----  ------------  ---------  --------
    money      money         8       NULL    NULL   1      NULL          NULL       0

    sp_help tid

    Type_name  Storage_type  Length  Prec    Scale  Nulls  Default_name  Rule_name  Identity
    ---------  ------------  ------  ------  -----  -----  ------------  ---------  --------
    tid        varchar       6       NULL    NULL   0      NULL          NULL       0
