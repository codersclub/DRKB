...
сообщение об ошибке:
    ...
    (Неправильная команда около слова 'count').
...
сообщение об ошибке. Далее приводятся примеры неправильных пакетов:
    ...
    CREATE VIEW must be the first command in a query batch.
    (Команда создания вьювера должна быть первой в пакете.)
    ...
    (Команда создания вьювера должна быть  единственной в пакете.)
...
Таблица 13-2: Сравнение неопределенных значений
    ...
    | с выражением          |                    |                |
    +-----------------------+--------------------+----------------+
    | Сравнение выражения   | TRUE               | FALSE          |
    | с табличным_значением |                    |                |
    +-----------------------+--------------------+----------------+
    | Сравнение выражения   | TRUE               | FALSE          |
    | с выражением          |                    |                |
    +-----------------------+--------------------+----------------+
...
Таблица 13-3: Глобальные переменные SQL Сервера
    ...
    | Переменная                | Значение                          |
    +---------------------------+-----------------------------------+
    | @@char_convert            | Равна 0, если не установлено      |
    |                           | преобразование алфавитов          |
    |                           | (множества символов). Равна 1,    |
    |                           | если такое преобразование         |
    |                           | установлено.                      |
    +---------------------------+-----------------------------------+
    | @@client_csname           | Содержит название алфавита        |
    ...
    |                           | алфавита.                         |
    +---------------------------+-----------------------------------+
    | @@client_csid             | Содержит идентификатор алфавита   |
    ...
    |                           | попыток регистраций.              |
    +---------------------------+-----------------------------------+
    | @@cpu_busy                | Равна времени (в тактах           |
    ...
    |                           | работы.                           |
    +---------------------------+-----------------------------------+
    | @@io_busy                 | Равна количеству времени (в       |
    |                           | тактах процессора), в течении     |
    ...
    +---------------------------+-----------------------------------+
    | @@max_connections         | Указывает на максимальное число   |
    ...
    +---------------------------+-----------------------------------+
    | @@pack_received           | Указывает на число входных        |
    |                           | пакетов (packet), прочитанных SQL |
    |                           | Сервером.                         |
    +---------------------------+-----------------------------------+
    | @@pack_sent               | Указывает на число выходных       |
    |                           | пакетов (packet), записанных SQL  |
    |                           | Сервером.                         |
    +---------------------------+-----------------------------------+
    | @@pack_errors             | Указывает на число ошибок,        |
    ...
    |                           | тип varbinary(8).                 |
    +---------------------------+-----------------------------------+
    | @@tresh_hysteresis        | Указывает на минимально           |
    ...
    |                           | данных SQL Сервером.              |
    +---------------------------+-----------------------------------+
    | @@total_read              | Указывает на число чтений с       |
    |                           | диска, которые SQL Сервер         |
    |                           | выполнил со времени последнего    |
    |                           | старта.                           |
    +---------------------------+-----------------------------------+
    | @@total_write             | Указывает на число записей с      |
    |                           | диска, которые SQL Сервер         |
    |                           | выполнил со времени последнего    |
    |                           | старта.                           |
    +---------------------------+-----------------------------------+
...
Команда raiserror имеет следующий вид:
    ...
    raiserror номер_ошибки
      [{форматированная_строка_вывода | @локальная_переменная }]
      [,список_аргументов]
