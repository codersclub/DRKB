...
можно применять и к вьюверам, о чем будет рассказано
в следующей главе 10: "Вьюверы: ограниченный доступ к данным".
...
> **Замечание:**
> Изменения, произведенные на удаленном SQL сервере с помощью
...
> **Замечание:**  
> Команда dateformat влияет только на
...
1) Если название месяца вводится в буквенном (символьном) виде, то действуют следующие соглашения:
...
2) Если название месяца, вводится в числовом виде, то оно отделяется от остальных частей даты косой(/),
   дефисом(-) или точкой (.), и действуют следующие соглашения:
   ...
   * Например, дату "April 15,1988" в формате mdy можно ввести в следующем виде:
   ...
   Далее приведены другие форматы ввода этой даты с разделяющей косой ("/"),
   в которых можно также использовать в качестве разделителей дефисы и точки:
...
3) Если дата задана как 4-, 6-, или 8-цифровая строка без разделителей, либо как пустая строка,
   либо указана только составляющая времени, то действуют следующие соглашения:
...
| Введенное значение | Мантисса  | Показатель | Величина        |
|-----------------|:---------:|:----------:|----------------:|
| 10Е2            | 10        | 2          | 10*10^2^         |
| 15.3е1          | 15.3      | 1          | 15.3*10^1^       |
| -2.е5           | -2        | 5          | -2*10^5^         |
...
Здесь приведен полный синтаксис команды insert:
    ...
    insert [into] [база данных.[владелец]] { название_таблицы | название_вьювера }
           [(список_столбцов)]
    {values (константное_выражение [, константное_выражение]...) | оператор_выбора }
...
| Значение по умолчанию? | Not NULL       | NULL            | Столбец-счетчик |
|-----------------|-----------------------|-----------------|-----------------|
| Да              | Значение по умолчанию | Значение по умолчанию | Следующее по порядку значение |
| Нет             | Сообщение об ошибке   | NULL            | Следующее по порядку значение |
...
установленные правила  с помощью системной процедуры sp\_helptext:
    ...
    as @pub_id in ("1389", "0736", "0877", "1622", "1756") or
...
> **Замечание:**
> Нельзя переписывать данные из столбца таблицы, в котором
...
> **Замечание:**  
> Ход выполнения команды обновления заносится в журнал
...
обновляемые данные задаются с помощью выражения:
    ...
    set название_столбца = выражение
    where условия_отбора
...
образом:
    ...
    set    [[[база_данных.]владелец.]{ название_таблицы. | название_вьювера. }]
           название _столбца1 = {выражение1 | null | (оператор_выбора) }
         [,название _столбца2 = {выражение2 | null | (оператор_выбора)
    }]...
    [from  [[база_данных.]владелец.]{ название_таблицы | название_вьювера }
        [, [[база_данных.]владелец.]{ название_таблицы | название_вьювера }]]...
    [where условия_отбора ]
...
> **Замечание:**  
> Перед тем, как начать выполнение примеров, приведенных в этом
...
**Замечание:**  
Чтобы команда writetext выполнялась в нормальном режиме, т.е.
...
записывается в уже существующую строку таблицы blurbs:
    ...
    writetext blurbs.copy @val  "This book is a must for true data junkies."
...
Упрощенный вариант синтаксиса команды delete выглядит следующим образом:
    ...
    where название_столбца = выражение
...
Синтаксис команды truncate table имеет следующий вид:
    ...
    truncate table [[база_данных.]владелец.] название_таблицы 
