---
Title: Язык XML. Стилевые таблицы XSL
Date: 01.01.2007
Source: <https://helper10.narod.ru/>
---
...
предназначенного для этого средства - стилевых таблиц XSL
...
программы-клиента(например, в окне броузера).
Предложенные в качестве рекомендация W3C, каскадные стилевые таблицы
(CSS - **C**ascading **S**tyle **S**heets [7])
уже больше года используются Web-разработчиками для оформления
...
[www.microsoft.com/xml/xsl/](www.microsoft.com/xml/xsl/)
...
Содержимое XSL-документа:
    ...
    <xsl>
      ...
      <rule>
        ...
        <td align="center">
         <p color="red" font-size="14" font-style="italic">
           <children/>
         </p>
        </td>
      </rule>
...
месторасположения внутри XML-документа:
    ...
    <xsl>
     ...
     <rule>
      ...
      <td align="center">
       <p color="blue" font-size="14" font-style="italic">
       <children/>
      </td>
     </rule>
    </xsl>
...
msxsl:
    ...
    |                             | "A" - A,..,Z,AA, .. ZZ            |
    +-----------------------------+-----------------------------------+
    | FormatNumberList(list,      | Возвращает строку, представляющую |
    |   format,separator)         | список, элементами которого       |
