...
Кузьменко Дмитрий, www.ibase.ru. 25.08.2000 (исправления от 14.12.2000,
17.08.2001).
...
Примечание: дальнейшая информация по датам взята из документа,
...
справка: драйверы ODBC 1.0 от Microsoft впервые появились в августе 1993
...
Данный рисунок и список файлов, возможно, развеет популярный миф о том,
...
Примечание: параметры SCHEMA CACHE не имеют абсолютно никакого отношения
...
Примечание: можно было бы отнести этот недостаток на SQL Link для IB, но
...
Примечание: разумеется, клиентская часть SQL-сервера может принимать
...
Для решения проблем кэширования TTable было введено два разных механизма
...
Сотрите все в SQL Monitor. И нажмите кнопку Button1. Теперь возвращаемся
к началу лога в SQL Monitor:
...
Если же курсор находится вверху или внизу грида, то вместо трех запросов
...
Примечание: вы сами можете убедиться в изложенном, поместив первый
...
TQuery работает с \"мертвым\" кэшем, поэтому обновлять его невозможно.
...
Примечание: Даже если компонент IBX IBTable и поддерживает Refresh, то
...
Примечание: в отличие от атомарного SELECT, оператор FOR SELECT внутри
...
BDE устроен так, что компонент TDatabase может работать только с одной
...
Примечание: иногда по неизвестным причинам BDE перечитывает запросы,
...
Примечание: найдена запись или нет, определяется анализом переменной
...
Два пользователя открывают таблицу. Один видит, что для данного имени
...
Примечание: если говорить о count, т.е. подсчете количества записей в
...
Однако, пусть даже и таким жутким способом, но BDE умеет выполнять
