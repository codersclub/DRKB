...
Заголовочный файл  ibase.h содержит следующее
...
иначе isc\_attach\_database() вернет код ошибки
*/
. . .
db1 = 0L;
db2 = 0L;
...
dpb++ = isc\_num\_buffers; /*говорит что далее пойдет описание параметра числа буферов*/
...
isc\_dpb\_user\_name | Имя пользователя
isc\_dpb\_password | Пароль
isc\_dpb\_password\_enc | Зашифрованный пароль
isc\_dpb\_sql\_role\_name | Название роли
isc\_dpb\_sys\_user\_name | Имя администратора системной БД
isc\_dpb\_license | Авторизированный ключ для лицензии
isc\_dpb\_encrypt\_key | Зашифрованный ключ БД

Управление средой

isc\_dpb\_num\_buffers | Число буферов кэша
isc\_dpb\_dbkey\_scope | dbkey context scope

Управление системой

isc\_dpb\_force\_write | Синхронная или асинхронная запись в БД (т.е. писать на диск сразу, или покоптить в памяти)
isc\_dpb\_no\_reserve | Определяет резервировать или нет небольшое место на каждой странице БД для хранения старых версий модифицированных записей когда модификации уже сделаны
isc\_dpb\_damaged | Определяет, должна БД быть помечена как поврежденная или нет
isc\_dpb\_verify | Выполнять последовательно проверку внутренних структур


Управление теневым файлом БД

isc\_dpb\_activate\_shadow | Активизировать теневой файл БД, который необязателен, но является синхро-копией БД
isc\_dpb\_delete\_shadow | Удалять теневой файл БД


Управление системой регистрациии

isc\_dpb\_begin\_log | Активизировать управление системой регистрации, для слежения за всеми запросами к БД
isc\_dpb\_quit\_log | Деактивировать систему регистрации


Спецификация файла сообщений и символьной кодировки

isc\_dpb\_lc\_messages | Языковая спецификация файла сообщений
isc\_dpb\_lc\_ctype | Используемая кодировка
...
dpb\_size                unsigned short*        Указатель на текущий размер DPB в байтах
...
base\_level               Версия БД ( 1 байт содержит число 1, 1 байт содержит номер версии)
...
счетчиков операций над базой данных:
  ...
  --------------- ---------------------------------------------------------------
  backout\_count                                Число удалений версии записи(?)
  --------------- ---------------------------------------------------------------
...
isc\_commit\_retaining()  | Потверждает изменения произведенные транзакцией и сохраняет контекст для дальнейшей обработки транзакции (то есть не заканчивает транзакцию. прим.пер)
isc\_commit\_transaction() | Подтверждает изменения произведенные транзакцией, и заканчивает транзакцию.
...
Функция                  Описание  isc\_start\_multiple()               Стартует транзакцию, если использеутся фортран вместо isc\_start\_transaction()
isc\_prepare\_transaction()                        Подготавливает первую стадию двухфазного
...
isc\_prepare\_transaction2()               Подготавливает первую стадию двухфазного
...
::: {style="text-align: left; text-indent: 0px; padding: 0px 0px 0px 0px; margin: 0px 0px 0px 24px;"}  1.   Создание и инициализация дескриптора транзакции для каждой одновременно стартующей транзакции.
...
назначаются по умолчанию если указатель на TPB установлен в NULL.
  ...
  Параметр                        Описание
  ---------------------------------- ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  isc\_tpb\_version3                                Версия 3 буфера транзакций
  ---------------------------------- ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  isc\_tpb\_consistency                  Модель транзакции "блокировка таблицы"
  ---------------------------------- ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
