---
Title: Hooks - аспекты реализации
...
в данном случае ShellHook - это и есть функция-фильтр. В дальнейшем, под
...
принимать одно из следующих значений:
    ...
    | **Константа**       | **Описание**                      |
    +---------------------+-----------------------------------+
    | WH_CALLWNDPROC      | Фильтр процедуры окна.            |
    ...
    +---------------------+-----------------------------------+
    | WH_CALLWNDPROCRET   | Функция-фильтр, контролирующая    |
    |                     | сообщения после их обработки      |
    |                     | процедурой окна приемника.        |
    +---------------------+-----------------------------------+
    | WH_CBT              | В литературе встречаются          |
    ...
    |                     | и клавиатуры.                     |
    +---------------------+-----------------------------------+
    | WH_DEBUG            | Функция-фильтр, предназначенная   |
    ...
    +---------------------+-----------------------------------+
    | WH_MOUSE            | Функция-фильтр, обрабатывающая    |
    ...
    +---------------------+-----------------------------------+
    | WH_MSGFILTER        | Функция-фильтр специального       |
    |                     | сообщения. Функция-фильтр ловушки |
    |                     | вызывается, когда сообщение       |
    |                     | должно быть обработано диалоговым |
    |                     | окном приложения, меню или окном  |
    |                     | приложения.                       |
    +---------------------+-----------------------------------+
    | WH_SHELL            | Фильтр приложения оболочки.       |
    |                     | Функция-фильтр ловушки            |
...
Чтобы упредить шквал писем в мой адрес, скажу сразу, что каждый, из
...
hmod
: данный параметр должен иметь значение hInstance в EXE или
...
dwThreadID
: данный параметр идентифицирует поток, с которым будет
...
: функция SetWindowsHookEx возвращает дескриптор
...
**Замечание:**
при установке двух ловушек разного типа, система создаст две
...
Запустите MainProg1.exe и MainProg2.exe, расположите
...
_Почему ?_ - спросите вы. А потому, что в АП Process2 (в данных DLL)
...
**Замечание:**
если вы работаете на Windows 2000, то вышеописанный пример
...
литературе, общие же понятия я постараюсь вкратце осветить.

Механизм файлов, отображённых в память (MMF - Memory Mapped Files) позволяет
...
совместные нужды двух процессов АП).

Данное приближение, вообще говоря,
...
подкачки Windows (файл страничного обмена), таким образом, нам не
...
обычным файлом, хранящимся на диске.

Таким образом, к рассмотренному
...
**Замечание:**
первым параметром функции CreateFileMapping() должен быть
...
Так как мы будем создавать именованный объект файлового отображения, то
