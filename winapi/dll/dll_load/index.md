...
продолжаю обсуждать вопросы разработки и использования DLL в Borland Delphi.

Для новых подписчиков сообщаю, что первую часть статьи они могут посмотреть в архиве рассылки,
выпуск номер 13.
...
так же, как если бы они были описаны внутри модулей приложения.
...
который имеется ссылка в приложении, отсутствует, программа откажется
загружаться.
...
библиотеки DLL должна быть выгружена с применением FreeLibrary.
...
файла проекта DLL (модуль с формой здесь не приводится), а второй - модуль
...
приложений, а также работы самих программистов.
