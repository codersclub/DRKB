---
Title: Практически всё о взломе CD
Author: lomovskih@yandex.ru
Date: 01.01.2007
...
Хотя сдесь и будет в основном теоретический материал, о способах защиты
от копирования с CD и методах устранения такой нехорошести, но я
...
`bpx GetDriveTypeA` и если после выхода в вызывающую программу, недалеко стоит
...
3. На эту идею я натолкнулся в статье Vadim M.
_"Как отучить игры от проверки CD-ROM (на примере Hellfire)"._
Краткое описание: поскольку
...
легко достичь переправив результат возвращаеммый функцией с 3 на 5,
правда это не всегда проходит. Эту статью можно увидеть на
www.reversing.net
...
**Теория:**
Метка тома проверяется с помощи функции GetVolumeInformation,
...
3. Если категорию 1 вы уже правили. Посмотреть метку тома на компакт-диске,
и установить такую же на любом жестком диске.
...
исполняемых файлах программы, настоящую метку и заменяем ее нашей.
...
**Теория:**
Наличие свободного места на диске проверяется функцией
...
**Теория:**
Один из распространившихся в последнее время приемчиков, часто
...
**Теория:**
Для этого используются функции WriteFile и GetLastError. Одна
...
**Теория:**
Бывают и другие сильно замусоленные проверки
...
разработчиков. Статья открыта для дополнения,
если у Вас есть какие-нибудь ценные мысли по этому поводу, шлите на мыло.
...
**P.S.**
Запомните все материалы публикуются только в учебных целях и автор
за их использование ответственности не несет!!

**P.P.S.**
Возможно имеют место опечатки, заранее извините!
