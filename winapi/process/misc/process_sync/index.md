...
состоянии, и последствия обращения к нему могут быть самыми неожиданными
-- от порчи данных, до нарушения защиты памяти.
...
Число в диапазоне от WAIT\_ABANDONED\_0 до WAIT\_ABANDONED\_0 + nCount
-- 1        Если bWaitAll равно TRUE -- это означает, что все перешли в
...
if GetLastError = ERROR\_ALREADY\_EXISTS then begin
...
Если lpTimerName совпадает с именем уже существующего в системе таймера
-- функция возвращает его идентификатор, позволяя использовать объект
