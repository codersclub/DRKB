...
функций Windows API, что описано в моей статье "Hooking Windows API".
...
эффективно или намного более просто.
...
они его пропускали. В случае, если объект - всего лишь возвращаемое значение
...
как использование возможностей файловой системы. К тому же это намного
проще, т.к. в этом случае нам не нужно знать как работает конкретная файловая
система.
...
файлов используется функция NtQueryDirectoryFile.
...
может быть получен с использованием функции NtOpenFile. FileInformation -
указатель на выделенную память, куда функция запишет необходимые данные.
...
элемента поле NextEntryOffset содержит нуль.
...
запись, нужно сдвинуть следующие за ней структуры на размер первой записи. Это
приведет к тому, что первая запись будет затерта. Если мы хотим скрыть другую
запись, мы можем просто изменить значение NextEntryOffset предыдущей записи.
Новое значение NextEntryOffset будет нуль, если мы хотим скрыть последнюю
запись, иначе значение будет суммой полей NextEntryOffset записи, которую мы
хотим скрыть и предыдущей записи. Затем необходимо изменить значение поля
Unknown предыдущей записи, которое предоставляет индекс для последующего поиска.

Значение поля Unknown предыдущей записи должно равняться значению поля
Unknown записи, которую мы хотим скрыть.
...
вычислить ее вручную. Мы можем сложить NextEntryOffset всех записей,
FileNameLength
...
Первый тип - ключи реестра, второй - значения реестра. Благодаря структуре
реестра скрытие его ключей не так тривиально, как скрытие файлов или процессов.
...
получать информацию о ключе с большим индексом запрашивая ключ с меньшим
индексом, мы
...
ключ E имеет индекс 4.

Теперь, если мы хотим скрыть ключ B и приложение вызвало
NtEnumerateKey с Index равным 4, мы должны вернуть информацию о ключе F,
так как индекс сдвинут.

Проблема в том, что нам неизвестно, что нужно произвести
сдвиг. А если мы не позаботимся о сдвиге, то вернем E вместо F, когда
будет запрашиваться ключ с индексом 4, или ничего не вернем для ключа с
индексом 1, хотя должны вернуть C.

В обоих случаях ошибка. Вот почему мы должны
подумать о сдвиге.
...
Поэтому мы должны придумать более совершенный метод.
...
описывают подключ, которых мы хотим открыть (включая его имя).
...
наконец, вызвать оригинальную NtEnumerateKey.
...
использовании KeyValueBasicInformation в качестве значения для
...
вынуждены перехватывать все четыре функции.
...
буфере зависит от типа функции. Для функций EnumServicesStatusA и
EnumServicesGroupW возвращается структура:
...
увидеть наши скрытые объекты до того, как функции будут перехвачены.
...
Чтобы установить сервис нам тоже нужны специальные привилегии.

Также очень полезно получение привилегии SeDebugPrivilege. Это может
...
описывается в разделе 3.2.3 статьи "Hooking Windows API".
...
ImageEntry - это точка входа модуля, ImageSize - его размер.
...
ImageBase мы сравним с ImageBase каждого элемента в
...
том, что функции будут использоваться для других процессов.
...
во втором списке и отсутствуют в первом. Но этот метод очень ненадежен.
...
сможем пропустить ни один новый процесс, используя данный метод.
...
процесса. Она вызвается после NtCreateThread.
...
какой процесс владеет данным потоком.
Мы должны просто проверить, заражен ли этот процесс.
Это можно определить прочитав первые байты любой из функций,
которые мы перехватываем.
...
размер этого буфера указан в параметре ThreadInformationLength.
...
ReturnLength должно быть установлено в нуль в данном случае.
...
байты (мы их сохранили) в буфер Buffer.
...
обнаружение выделенной нами памяти.
...
MEM\_FREE, следует прибавить размер следующей секции к RegionSize.
...
20 бит - это старшие 20 бит адреса секции, а младшие 12 бит содержат флаги.
...
процесса значение поля Handle растет.
...
срабатывает довольно редко, поэтому мы можем оставаться спокойными и использовать
его.
...
Параметр DesiredAccess должен быть равен PROCESS\_QUERY\_INFORMATION,
а Attribures и Options - нулю.
...
для скрытия портов - это NtDeviceIoControlFile.
...
самое в OutputBuffer и OutputbufferLength.

**Netstat, OpPorts в WinXP, FPort в WinXP**

Получение списка открытых портов в первую очередь используется,
...
\\Device\\Udp - это хэндл открытого порта.
...
https://rootkit.host.sk.

Сейчас нам необходимо скрыть определенные
порты. Мы
...
на https://www.rootkit.com.

Возможно, я добавлю информацию о невидимости в Windows NT в будущем.

Новые версии документа также будут содержать улучшения описанных методов
и новые комментарии.
