...
> Требуется нажать в "другом" приложении пару кнопок (button).
> (кнопки не имеют hotkeys). Ищу окно так (Дельфи):
> 
>     if FindWindow(nil, 'Advanced Dialer')<> 0 then
>       ShowMessage('OK');
> 
> А теперь в найденном приложении надо нажать кнопку HangUp, подождать 5
> сек. и нажать кнопку Dial. Подскажите плз. как это реализовать?

Я не знаю, что там за кнопки...
Если класс Button, то один вариант, если это кнопки на тулбаре, то другой.
Вот посмотри, я писал когда-то, лишнее стирать лень...
реализуется 1-й и 2-й способ:
