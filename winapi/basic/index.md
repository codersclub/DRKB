...
Такая запись обеспечивает одновременно и статическое подключение
...
обычными типами Delphi.

Кроме этих типов общего назначения существуют
...
В некоторых именах типов, встречающихся в справке, и
...
определённым в Graphics.dcu.

Зачем разработчикам Delphi потребовалось
называть разные типы одним именем - непонятно, тем более что во второй
...
Если посмотреть справку, например, по функции GetSystemMetrics, то
...
просматривая Windows.pas, можно узнать, что SM\_ARRANGE = 56.
...
Иногда напротив названия одной из систем стоит слово,
...
из Microsoft вопрос, зачем нужны такие пни.

Overview - это краткий обзор
...
туда стоит только если ну совсем ничего непонятно.
...
Иногда в справке можно встретить указания или "Windows 95 оnlу" или "Windows NТ оnlу".
К этим замечаниям
...
Ещё несколько слов о числовых константах. В справке можно встретить
...
должен сам позаботиться о "выносе мусора". Аналогичная ситуация и с
...
необходимости их можно удалить и самостоятельно.
...
Создание окон через Win API требует кропотливой работы. VCL Delphi
...
Еще одно общее свойство - конкретное имя
...
В 16-разрядных версиях Windows вызов callback функций осложнялся тем,
...
В Delphi для реакции на каждое событие обычно создаётся свой метод. В
...
`WM_`: например, WM\_Paint, WM\_GetTextLength.

Сообщения, специфичные,
...
Буквы и означают и, то есть первый параметр 16-разрядный,
а второй - 32-разрядный. Однако так было только в старых, 16-разрядных версиях
...
Отправка сообщения другому окну может осуществляться достаточно
...
обработки какого-то конкретного сообщения может и не существовать. Это,
...
Так как параметры WParam и LParam могут иметь совершенно различный смысл
...
Прежде чем зарегистрировать сообщение, необходимо придумать ему имя
...
Метод компонента не может быть оконной процедурой, потому что методу
...
Вызов процедуры HookOwner я обычно помещаю в самый конец конструктора
...
Для получения дескриптора контекста устройства существует много функций.
...
При переходе на 32-разрядную версию Windows многие функции были
...
Получить строку от функции Win API несколько сложнее, чем передать её.
...
Всё. Если человек не готов часами и даже
