...
перехвата уже разобрался :-)
_[Енота: разобраться-то он действительно разобрался, а соксифиера нет до сих пор...]_
...
_[Енота: мечтатель, однако...]_ я решил, что придется заводить список
...
классика :-) - так люди и делают. ну он большой, ему виднее :-) }
    ...
    { как системе обрабатывать событие в ContinueDebugEvent. обнаружилось,
      что если это событие - не исключение (EXCEPTION_DEBUG_EVENT), то этот
      флажок системе "по сараю". а если исключение, то есть два варианта:
        DBG_CONTINUE - наш "отладчик" успешно обработал все сам, и
        DBG_EXCEPTION_NOT_HANDLED, что значиит - передать исключение системе
      на обработку }
    ...
    { флажок, который указывает обработчику события EXCEPTION_SINGLE_STEP
    ...
    { флажок, указывающий, завершился ли отлаживаемый процесс.
      Sleepyhead говорит, что иногда процесс не завершается корректно
      (к примеру, отладчик, который отлаживает отладчик, который отлаживает отладчик...
...
Все. Не так страшен черт, как его малюют
[Енота: или: не так страшен Гейтс... :-)].
Остались мелочи.
