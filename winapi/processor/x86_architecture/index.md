...
видели - это такие длинные узкие текстолитовые «планочки»
(фактически - небольшие платы) с некоторым количеством микросхем на них,
...
64-битными каналами, но на разрядность шины памяти это никак не
...
обеспечить скорость чтения и записи информации, равную 8 байт
(ширина шины) \* 2 (количество каналов) \* 2 (протокол DDR, обеспечивающий
...
только от неё самой, но и от контроллера памяти
и места его расположения - все эти факторы тоже влияют на задержку.
Именно поэтому компания AMD
в процессе разработки архитектуры AMD64 решила «одним махом» решить
проблему высокой латентности, интегрировав контроллер
прямо в процессор - чтобы максимально «сократить дистанцию» между процессорным ядром и
...
(не от Intel) расширенные наборы команд: EMMX, 3DNow! и
Extended 3DNow! - их разработала компания AMD. Впрочем, «официальность» и
...
Для чего предназначены дополнительные наборы команд?
В первую очередь - для увеличения быстродействия при выполнении некоторых операций.
...
Таким образом, понятие «архитектуры» применительно к
...
маркам какого-либо производителя.

Де-факто, употребление одного из трёх,
...
вместо 32-битного 64-битный формат).

Количество маркетинговых спекуляций
...
AMD64» не выдерживают никакой критики. И вовсе не потому,
что это не так - а потому, что вовсе не «нагло». Есть такое понятие:
...
Кстати: а почему мы употребили термин «64-битная
...
всём при том, что формально процессоры данной архитектуры достаточно
...
В общем, все всё прекрасно понимали, но AMD старательно делала
...
промежуточные соответствия.

Мотивация, которой руководствовалась Intel,
...
способна передавать за такт.

По обычной процессорной шине с «одинарной»
...
Впрочем, самое главное здесь - выучить наизусть один
...
А вот сам запрос на данные посылается по шине с «одинарной»
скоростью - всегда! Соответственно, если запросов у нас много, а размер
...
Во всех современных процессорах есть кэш (по-английски - cache).
Кэш - это некая особенная разновидность памяти (основная особенность,
...
В общем случае, именно так и работает процессорный кэш:
...
структуру L2, и в современных x86 CPU встречается редко. Чаще всего,
L3 - это плод компромисса: за счёт использование более медленной и узкой
...
зависит от скорости работы декодера.

Достаточно необычный способ
...
единственно необходимым элементом процессора.

Можно обойтись без кэша - скорость снизится, но программы работать будут.

Можно обойтись без декодера - исполняющие устройства станут сложнее, но работать
процессор будет. В конце концов, ранние процессоры архитектуры x86
(i8086, i80186, 286, 386, 486, Am5x86) - как-то без декодера
обходились.

Без исполняющих устройств обойтись невозможно, ибо именно
...
Блоков ALU в современных процессорах, как правило, несколько.
...
передаются по шине за один такт.

Количество передаваемых за такт байтов
...
передачу и приём информации.

В духе «здорового гигантизма» в очередной
...
объёмах L1/L2 у AMD общий размер кэшируемой информации получается
больше - но за счёт неё же он работает медленней (задержки, вызванные
...
данную последовательность в уже декодированном виде.
...
Таким образом, если принять время исполнения каждой
...
декодировать и исполнять (см. внеочередное исполнение).
...
единицу. Соответственно, до тех пор, пока значение счётчика больше
нуля - осуществляется переход на начало цикла, а после того, как он
становится равным нулю - исполнение продолжается дальше.
...
дополнительная нагрузка на контроллер памяти (де-факто,
в случае ошибки - совершенно бесполезная).
...
_(изложение личной позиции одним из близких знакомых автора)_
