...
и Icon - сама иконка с которой будет делаться эта операция
...
Данный опус писался только в форме, в IDE не тестировался.
Всё работает конечно, но не обессудьте, если будут ошибочки мелкие.
...
Для работы с SystemTray существует всего одна функция.
Вот её Си-прототип:
...
- uID - уникальный идентификатор иконки. Идентификатор
  должен быть уникален в пределах окна-обработчика,
  передаваемого в hWnd.
- uFlags - битовое поле, определяющее какое из следующих
  ...
  NIF\_ICON, NIF\_MESSAGE, NIF\_TIP или их OR-комбинацией.
