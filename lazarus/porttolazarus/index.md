...
программирования общего назначения. Примерная схема архитектуры представлена на рисунке
...
Наше приложение является многопоточным, поэтому сразу возник также
...
Если в frm -файле никакой экзотики которой нет в данной сборке нет, то
...
7.  **Если внутри SO (DLL) есть форма, которая должна самоуничтожаться,
	...
	вызова обработки сообщений из центрального цикла. Внутри DLL\SO
	надо сделать так:
	
```	
//Цикл сообщений для обработки внешних сообщений, т.к. иначе внутри плагинов не закрываются окна !
...
```	
	И при создании главной формы сделать Application.OnIdle := HandlePluginsMessages;
