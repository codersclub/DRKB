...
CGI-функции могут быть реализованы на сервере в двух вариантах: в форме
...
Кроме возможности создания CGI-приложений "с нуля", Borland Kylix
...
`http://localhost/cgi-bin/frenchwines`.

При этом в окне броузера появится
...
Заголовок обработчика выглядит следующим образом:
    ...
    type THTTPMethodEvent = procedure (
                       Sender: TObject;
                       Request: TWebRequest;
...
OnAction:
    ...
    begin
      ...
      Response.Content := '<HTML><BODY>' +
                          '<H2>This page is redirected to ' +
                          '<A HREF='+ Response.Location + '/>' +
                          Request.Host + Response.Location + '</A>' +
                          '</H2></BODY></HTML>';
    end;
...
действия может выглядеть следующим образом:
    ...
    begin
      ...
      Response. Content := '<HTML><BODY><H2>Error 404</H2>'
                         + '<P>Requested resource "' 
                         + Request.InternalScriptName+Request.PathInfo
                         + '" is not found on this server.</BODY></HTML>';
    end;
...
Компонент TPageProducer генерирует страницу на основе заданного шаблона.
...
Значения переменных ModuleName и ContentType необходимы серверу Apache
...
Далее внесите следующие строки в файл httpd.conf:
...
Web-сервере размещены файлы с исходными текстами Kylix-программ
(файлы \*.pas и \*.dpr) и вы хотите, чтобы пользователям эти файлы передавались
