...
(применяйте двойное подчеркивание в
секции объявления прототипа функции

    extern "C" { ... }

(вместо этого можно также использовать `__declspec(dllimport)`...).
...
функций `__stcall`, так что Ваша Delphi DLL соответственно должна
...
прописанных в разделе экспорта.

Для примера, если Вы экспортируете функцию

    function plusone (intval : Integer),

то Вам необходимо включить следующую строку в раздел экспорта .DPR-файла:
...
порядковые номера всех экспортируемых DLL функций.

Формат .DEF-файла очень прост:
...
параметра подставляете имя .DEF-файла. Например,
