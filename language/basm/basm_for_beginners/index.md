...
Date: 12.12.2003
Source: borland.public.Delphi.languages.basm <news://forums.borland.com>
---
...
Источник: borland.public.Delphi.languages.basm (news://forums.borland.com)
...
простой, умножение на 2 заменяется сложением операнда с самим собой:

    I + I = 2I

Инструкция RET возвращает управление в строку, следующую за
вызовом функции.
...
через регистры). Функция `MulInt2_BASM2` вызывается, и возвращает свой
результат в регистре `EAX`, который в следующей строке копируется в
регистр `ESI`.

**Замечание об оптимизации:**
Умножение на два может быть сделано двумя
...
латентности (latency) и по пропускной способности (throughput).

> От переводчика:  
> в дальнейшем в документе будут использоваться термины - latency
> и throughput без перевода или латентность, поскольку нет
...
Попробую объяснить ассемблерный код, строка за строкой.
Код `begin` выглядит в коде так.
...
запятой, данный код очень прост для понимания. В первой строке,
`FLD QWORD PTR [A]`, загружается константа A в регистр стека с плавающей
запятой. Строка `FMUL QWORD PTR [EBP+$08]` умножает A на X. Это
понятно при просмотре Паскаль кода, но что означает
`QWORD PTR [EBP+$08]`.
QWORD PTR означает "указатель на двойное слово, которое
...
эффективной. Следующие три строки не требуют специального пояснения,
но инструкция `FADDP ST(1)` нуждается в объяснении. Все
...
который является вторым, поскольку первый регистр это `ST(0)`!
Регистры с
...
стека. Таким путем результат помещается в `ST(0)`. Строка `FADD QWORD PTR [C]`
заканчивает вычисление, и единственная вещь, которая осталась, это
...
Осталось также объяснить, что скрывается за `[A]` в строке `fld qword ptr [A]`.
Мы знаем, что `A` должен быть указателем на место, где хранится
само `A` в памяти. Адрес `A` закодирован в инструкции. Вот полная строка из
окна CPU.

    00451E40 DD05803C4500     fld qword ptr [B]

`00451E40` это адрес инструкции в exe файле.
`DD05803C4500` это машинный код строки `FLD QWORD PTR [B]`,
которая более понятна для человеческого разума.
При просмотре руководства @Intel SW Developers Manual Volume 2@,
страница 280, мы увидим, что код команды для `FLD` равен `D9, DD, DB` или
`D9C0`, в зависимости от типа данных. Мы узнаем, что `DD` это код для
`FLD DOUBLE`. Остается еще `05803C4500`. `05` это (Не знаю, может быть, кто-то
...
**Второй сюрприз.**
Вызов функции с `X = 2` должен рассчитать `Y = 2^2+2*2+3 = 11`.

Но SecondOrderPolynomial3 возвращает `3`!
Снова открываем окно просмотра FPU, так как окно CPU и трассируем код,
наблюдая, что происходит.
Видно, что `A=1` корректно загружается в ST(0) в строке 4, но
в строке 5, которая производит умножение A на X, 1 на 2, результат в
ST(0) что-то очень маленький, в действительности `0`.
Это означает, что X близок к 0 вместо 2.

Могут быть неверными две вещи. Вызывающий код
...
видим, что он одинаков и поэтому не может быть причиной ошибки.

Было бы большим сюрпризом, если бы Delphi делала это неверно!
Пробуем опять трассировать код вызова, наблюдая за окном просмотра памяти в окне
...
Два указателя помещаются на стек. Один из них это указатель на `X`.
Но что за второй указатель? Просматриваем окно памяти и видим, что первый
...
Во-первых, в первой строке загружаем `X`.
Во второй строке загружаем `A`.
В третьей строке умножаем `A` на `X`.
В четвертой строке умножаем a\*X, расположено в ST(0) на `X`.
Так мы выполнили первое вычисление.

Загружаем `B` и умножаем его на `X`, этим
...
вычисления 1 и 2 и выкидываем вычисление 2 из стека. Единственное, что
...
арифметический сопроцессор, одна из них это 1 которая может быть
загружена инструкцией `fld1`. Использование её убирает одну загрузку из
...
перенос и 1 в младшем разряде. Затем мы складываем
`Перенос + 1 + 3 = 1 + 1 + 3 = 5`. Результат равен 51.
В третьем примере мы рассмотрим случай с
переполнением. `50 + 51 = 101`. 101 слишком велико, что бы разместиться в
двух разрядах и наш «CPU» не сможет выполнить расчет. Здесь также
получился перенос при сложении двух старших цифр.

Вернем в код. Могут произойти две вещи. Если мы компилировали без проверки диапазонов, то
...
Теперь подумаем так:
Если бы мы имели 64-битные регистры, то сложение
...
называется `MOVQ`. Символ `Q` означает `QUAD WORD` (четыре слова).
`MMX` именуются, как `MM0, MM1...MM7`.
В отличие от регистров FPU они не
...
Затем взглянем на инструкцию сложения, которая выглядит так: `PADDQ`.
`P` означает MMX, `ADD` означает сложение, а `Q` означает `QUAD WORD`.
И снова мы в недоумении, поскольку здесь нет таких MMX инструкций.

А что насчет SSE? Опять разочарование. В конце концов, SSE2 имеет это и мы счастливы
...
EMMS требует 23 такта. Сложение только 2 цикла. Перегрузка налицо.
...
инструкций. `FSTSW` записывает флаги `FP` в регистр `AX`, а `SAHF` копирует 8-бит
из регистра `AH` в регистр `EFlags`. Это длинный путь для флагов, перед тем
как они смогут быть использованы в инструкции `JNB`. JNB означает
`JUMP NOT BELOW` (переход если не меньше). В руководстве «Intel SW Developers
...
(CF=1) и флаг нуля (ZF=1). Попробуйте протрассировать код просмотром в
...
А это строка на 8 байт далее её
...
готовы к выполнению тела цикла.
...
сравнении `I` со `Stop`. Но это также и увеличивает стоимость инструкции
`DEC NoName`. На P4 latency/throughput инструкции CMP составляет 0.5/0.5
цикла, а для DEC оно 1/0.5. Поэтому это можно считать «де оптимизацией».
Значения latency и throughput для P4 можно найти в руководстве «Intel Pentium 4 and
Xeon Processor Optimization» от Intel.
...
загружают переменную `I` в регистр `EAX` и затем прибавляют её к переменной
...
оптимизированным один. Проверка вверху, то что проверяется всегда.
`Start` был на `Stop` и теперь лишнее и поэтому удалено. Перед проведением
...
использования. После того, как `Start` назначен переменной `I`
(`MOV [EBP-$10], EAX`), он больше нигде не используется и регистр EAX
...
так мы поступали много раз выше. Удаление лишних строк кода,
`MOV EAX, EAX`, был примером удаления мертвого кода, что является базисом для любых
оптимизаторов.
...
Поэтому одна пересылка EAX прямо в ECX может заменить эти две
...
инструкции». Дополнительная латентность, которую мы имеет:
`0.5 + X + 0.5 + X = ?` циклов.
...
выполняться параллельно. Поскольку за инструкцией `DEC NoName` не следует
...
итерации для этого необходима инструкция `CMP EDX,0`.
...
Конечно, значения счётчика циклов (clock count) должны быть целыми.
Но на P4 возможны пол цикла, в связи с double-clocked ALU.
Выходит, что наши измерения не так хороши, как хотелось бы,
но для сравнения производительности циклов они вполне достаточны.

**Заключение для P4.** Используйте только оптимизацию "No Loop Inversion" или
"loop inversion with NoName variable".

**Заключение для P3.** Не используйте "No Loop Inversion but NoName variable"
оптимизацию.

**Заключение для обеих платформ.**
Используйте обе оптимизации как делает Delphi.
...
Это урок 6 и его тема `CharPos`.

Данная функция ищет первое вхождение
...
`Pos` как подстроки возможна и это путь использования `Pos` как `CharPos`.
В данном уроке мы разработаем `CharPos`, которая будет примерно в 4 раза
быстрее, чем `Pos`.
...
передается как константа. Результат работы функции типа `Cardinal`.
В начале в функции проверяется, что входная строка не пустая и если
...
В данном тесте `CharPos1` получила результат 767 на P4 1600A, разогнанный
до 1920 и `CharPos2` получила результат 791. Для сравнения Delphi `Pos`
получила результат всего 2637.

Поскольку код `CharPos1` незначительно лучше, чем `CharPos2`,
то мы выбрали именно его для дальнейшей оптимизации.
Это ассемблерный код на Delphi 6,
...
копируется в EDX и сравнивается с 0. `TEST EDX, EDX` - то же самое, что и
...
В строке 4 параметр `Chr` был скопирован в регистр EBX, но тип `char` имеет размер
только в один байт. Поэтому первая инструкция CMP сравнивает, что в
...
Если указатель, переданный через EAX, в функцию `LStrLen` имеет значение nil,
то ничего не делается, а просто производится возврат из функции. Если же
...
`CharPos`, а это не то, что мы хотим. А вот так наша встроенная (inline)
...
длину точно так же, как первая строка во встроенной `LStrLen`.
Проверки `Str` на `nil` вполне достаточно ;-).

Вторая строка удалена, а первая изменена,
...
копируется в EDX. `MOV EAX, [ESI-$04]`. Это можно изменить на
`MOV EDX, [ESI-$04]`, а инструкцию `MOV EDX, EAX` можно удалить.
...
которую изменим на `CMP AL, [ESI+ECX]`. Затем удалим ненужное теперь
`MOV EBX, EAX`. Это устранение лишнего копирования и удаление мертвого кода и
...
закончился цикл. Данная строка порождала лишнюю инструкцию `CMP EBX, ECX`,
...
изменим, эти два перехода так, чтобы только один из них шел на `If2`,
то мы сможем удалить лишнюю проверку. Вместо перехода на `If2` при
...
спектакль. Появляется необоснованная тенденция думать, что P4 имеет
худший дизайн, но это не подтверждается нашим кодом. Взяв
победителя для смешанной платформы и сделав приведение результата к
процессору с частотой 1400 MHz, мы получим `950` для P3 и `710 * (1920/1400) = 973` для
...
Добро пожаловать на урок номер 7.

Темой сегодняшнего урока является плавающая запятая в BASM.
Это уже было темой в более раннем уроке, но
...
используется в аппроксимации. Это потому, что выражение второго порядка 
...
называемый `FP_ADD`, который предназначен для операций сложения и
...
обработки в конвейере `F_MUL`. Латентность для FMUL на P4 составляет 7
...
конвейер `F_ADD` прибавит значение `D`. В такте 15 будет закончено
вычисление `(A*X)*X` и можно будет начинать выражение `(A*X*X)*X`.
В такте 17 выражения `(B*X)*X` и `(C*X) + D `будут закончены и можно начать
работу с конвейером `F_ADD`. Данное сложение будет закончено на такте 21,
...
конвейер F\_MUL, с 7 состояниями, а правая конвейер F\_ADD на 5
состояний.
...
:---:|:-----------:|:-----:
 ...
 6   | C\*X<br>B\*X<br>A\*X |
 7   | C\*X<br>B\*X<br>A\*X |
 8   | (A\*X)\*X<br>C\*X<br>B\*X |
 9   | (A\*X)\*X<br>C\*X<br>B\*X |
 10  | (B\*X)\*X<br>(A\*X)\*X<br>C\*X |
 11  | (B\*X)\*X<br>(A\*X)\*X<br>C\*X |
 12  | (B\*X)\*X<br>(A\*X)\*X | (C\*X)+D
 13  | (B\*X)\*X<br>(A\*X)\*X | (C\*X)+D
 14  | (B\*X)\*X<br>(A\*X)\*X | (C\*X)+D
 15  | (A\*X\*X)\*X<br>(B\*X)\*X | (C\*X)+D
 16  | (A\*X\*X)\*X<br>(B\*X)\*X | (C\*X)+D
 17  | (A\*X\*X)\*X | (B\*X\*X)+(C\*X+D)
 18  | (A\*X\*X)\*X | (B\*X\*X)+(C\*X+D)
 19  | (A\*X\*X)\*X | (B\*X\*X)+(C\*X+D)
 20  | (A\*X\*X)\*X | (B\*X\*X)+(C\*X+D)
 21  | (A\*X\*X)\*X | (B\*X\*X)+(C\*X+D)
 22  |           | (A\*X\*X\*X)+ (B\*X\*X+C\*X+D)
 23  |           | (A\*X\*X\*X)+ (B\*X\*X+C\*X+D)
 24  |           | (A\*X\*X\*X)+ (B\*X\*X+C\*X+D)
 25  |           | (A\*X\*X\*X)+ (B\*X\*X+C\*X+D)
 26  |           | (A\*X\*X\*X)+ (B\*X\*X+C\*X+D)
 27  |           | Finished
...
загрузка `Error` излишние и оптимизатор должен удалить это. `FABS` это
...
массива в EAX. Вторая строка загружает элемент _I_, который находится в
...
загрузке переменной, она может оказаться разделенной между двумя строками
кэш памяти, что потребует двойного времени на ее загрузку. Поскольку
переменные двойной точности имеют размер в 8 байт, а строка кэша L1 на P4
размером в 64 байта, то одна из восьми переменных может оказаться
разнесенной по разным строкам. На P3 ширина кэша L1 составляет 32 байта,
...
4 байта, а восьми-байтные - на границу в восемь байт соответственно. Чтобы
...
по адресу 0, так что память из адреса 0 будет загружена в неё. Наша
...
восьмая по байту 61. Первые её 4 байта находятся в строке 1, но
...
Компилятор вставил строку `MOV EBP, ESP`, и мы можем уменьшить
...
стека. Эта одиночная строка имеет то же действие, но избыточность
удалена.
...
его из стека, и удалим строку, которая делает это.
...
Вместо написания всех строк `QWORD PTR [ESP+$XX]` мы можем писать имена
...
Добавленная вторая строка загружает `X` один раз, для всех операций.
...
3%. `FXCH` действительно ни при чём, поскольку производительность опять
ушла вниз. В чём же дело?
...
С другой стороны мы можем также сказать, что получение данных из
кэш-памяти всегда медленнее, чем получение из внутренних регистров. P4 имеет
...
Мы видим, что в результате «оптимизаций» на шагах от `d` до `i` мы
...
На P3 победитель - версия 4, но с меньшим преимуществом.
...
в данном наборе - это использование `SIMD` расчетов. **SIMD** - это
аббревиатура для **S**ingle **I**nstruction **M**ultiple **D**ata.
«Множество данных» (Multiple data) здесь - это переменные двойной точности с плавающей
...
точности, при копировании в переменную.

SSE2 вычисления с другой стороны
...
throughput для инструкции _MULSD_!
...
кажется, что SIMD версия `ADD`, `ADDPD`, имеет ту же самую латентность и
...
`Xmm0` используется как источник данных в каждой строке, но эта строка не
...
неудобно, и я с надеждой смотрю в сторону Борланд на тему создания окна
...
это, чтобы снять различие между SSE2 и FP кодом.
...
что бы они работали с корректной парой переменная/регистр. Активируя
...
означает **S**calar-**D**ouble (Скаляр - Двойная точность).
...
получила 48292 пункта. Мы должны разобраться, в чем причина этого. То ли
причина в перегрузке в двух последних строках, то ли в 2-тактном
...
И теперь вы знаете почти всё о программировании с плавающей запятой ;-)
