<h1>Получаем из реестра количество активных потоков, загруженность процессора и т.д.</h1>
<div class="date">01.01.2007</div>

Автор: Vimil Saju  </p>
<p>В реестре есть раздел HKEY_DYN_DATA. Основная информация о системе хранится в ключе PerfStats. </p>
<p>О получении информации,например, о загруженности процессора, необходимо проделать следующие шаги: </p>
<p>Для начала необходимо запустить установленный счётчик в реестре. Это возможно путём считывания значения ключа, отвечающего за нужный параметр системы. </p>
<p>Например </p>
<p>Просто считываем значение ключа 'PerfStats\StartStat\KERNEL\CPUusage' в секции HKEY_DYN_DATA. данное действие запускает счётчик. После этого в ключе 'PerfStats\StatData\KERNEL\CPUusage' будет храниться значение в процентах о загруженности процессора. </p>
<p>Далее, если добавить считывание загруженности процессора в событие On timer, то мы сможем наблюдать изменение загруженности процессора в динамике. </p>
<p>По завершении, Ваша программа должна остановить счётчик в реестре. Для этого просто считай ключ 'PerfStats\StopStat\KERNEL\CPUusage'.Это остановит счётчик. </p>
<p>Так же в системе есть много других счётчиков. Весь список счётчиков можно посмотреть в ключе PerfStats\StatData, используя редактор реестра. </p>
<p>Представленный ниже исходник получает значения всех счётчиков, расположенных в секции HKEY_DYN_DATA. </p>
<pre>unit SystemInfo;
...
<p>Скопируйте это в файл .pas и проинсталлируйте его. </p>
