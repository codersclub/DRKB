...
В настоящее время в конференциях то и дело встречаются вопросы типа: как
мне получить изображение со сканера, с web камеры и т.д.. При том, что и
...
Открыть диалог настройки соответствующего устройства (диалог отображает
DS) и задать соответствующие настройки.

Приложение ожидает сообщение от DS, что изображение готово. Во время
...
TWAIN\_SelectImageSource(hwnd: HWND): Integer;
...
TWAIN\_AcquireNative(hwnd: HWND; pixmask: Integer): HBitmap;
...
TWAIN\_FreeNative(hDIB: HBitmap);
...
Полный текст примера можно взять отсюда. Мы рассмотрим только функцию
