<h1>Как работать со сканером?</h1>
...
<hr />The setup program for Imaging (tool that ships with Windows &gt; 98) installs the Image</p>
<p>Scan control (OCX) and the 32-bit TWAIN DLLs.</p>
<p>All you have to do is to import this ActiveX control in Delphi and generate a component wrapper:</p>

<p>Import the ActiveX Control "Kodak Image Scan Control"</p>
<p>(Select Component|Import ActiveX Control...)</p>

<p>Now add a TImgScan Component from the Register "ActiveX" to your form.</p>

<p>Change the following Properties in the Object Inspector:</p>

<p>FileType = 3 - BMP_Bitmap</p>
<p>PageOption = 4 - OverwritePages</p>
<p>ScanTo = 2 - FileOnly</p>
...
<p>В настоящее время в конференциях то и дело встречаются вопросы типа: как мне получить изображение со сканера, с web камеры и т.д.. При том, что и интернете практически полностью отсутствуют материалы по этим вопросам на русском языке и при достаточном разнообразии их на английском. Эта статья должна помочь начинающему программисту на Delphi разобраться в них. В статье подробно, с примерами описана работа со сканером с использованием популярной библиотеки Easy TWAIN.</p>

<p>Введение</p>

<p>В отличие от принтеров сканеры изначально не поддерживались ОС Windows и не имеют API для работы с ними. В начале своего появления сканеры взаимодействовали с программами посредством уникального для каждой модели сканера интерфейса, что серьезно затрудняло включение поддержки работы со сканером в прикладные программы.</p>

<p>Для решения этой проблемы был разработан TWAIN - индустриальный стандарт интерфейса программного обеспечения для передачи изображений из различных устройств в Windows и Macintosh. Стандарт издан и поддерживается TWAIN рабочей группой - официальный сайт www.twain.org. Стандарт издан в 1992 г. В настоящее время действует версия 1.9 от января 2000 г. Абревеатура TWAIN изначально не имела какого-то определенного смысла хотя позже была придумана расшифровка: (Technology Without An Interesting Name - Технология без интересного имени). TWAIN - не протокол аппаратного уровня, он требует драйвера (названного Data Source или DS) для каждого устройства.</p>

<p>К настоящему времени (май 2000 г.) TWAIN доступен для Windows 3.1 и выше (Intel и совместимые процессоры), Macintosh и OS/2. Для Linux самый близкий стандарт - SANE.</p>

<p>Менеджер TWAIN (DSM) - действует как координатор между приложениями и Источником Данных (Data Source). DSM имеет минимальный пользовательский интерфейс - только выбор DS. Все взаимодействие с пользователем вне прикладной программы осуществляется по средствам DS.</p>

<p>Каждый источник данных разрабатывается непосредственно производителем соответствующих устройств. И их поддержка стандарта TWAIN осуществляется на добровольной основе.</p>

<p>Использование TWAIN</p>

<p>DSM и DS это DLLs загружаемые в адресное пространство приложения и работают как подпрограммы приложения. DSM использует межпроцесcную связь, что бы координировать действия со своими копиями, когда больше чем одна программа использует TWAIN.</p>

<p>Упрощенная схема действия приложения использующего TWAIN:</p>

<p>Открыть диалог настройки соответствующего устройства (диалог отображает DS) и задать соответствующие настройки.</p>
<p>Приложение ожидает сообщение от DS, что изображение готово. Во время ожидания все зарегистрированные сообщения будут направляться через TWAIN. Если это не будет выполняться, то приложение не получит сообщения о готовности изображения.</p>
<p>Приложение принимает изображение от DS.</p>
<p>TWAIN определяет три типа передачи изображения:</p>
<p>Native - в Windows это DIB в памяти</p>
<p>Memory - как блоки пикселей в буферах памяти</p>
<p>File - DS записывает изображение непосредственно в файл (не обязательно поддерживается)</p>
<p>Приложение закрывает DS.</p>
<p>Использование EZTWAIN</p>

<p>Данная библиотека была разработана, что бы упростить разработку программ использующих TWAIN предоставляя разработчику упрощенную версию TWAIN API.</p>

<p>EZTWAN обеспечивает передачу всех windows сообщений через TWAIN и ожидает сообщения о готовности изображения.</p>

<p>Библиотека EZTWAIN является свободно распространяемой библиотекой с открытыми исходными кодами. В настоящее время выпущена версия 1.12. Библиотеку можно свободно скачать с сайта: www.dosadi.com, библиотека написана на C и предназначена для использования как DLL, необходимый для ее использования с Delphi модуль так же можно скачать с сайта. Кроме нее у меня с сайта можно скачать модификацию данной библиотеки, предназначенную для статической компоновки с программой на Delphi. Указанная версия (MultiTWAIN for Delphi) не требует наличия библиотеки EZTW32.DLL.</p>

<p>Структура программы</p>

<p>Используемые функции.</p>

<p>Перед вызовом функций сканирования необходимо вызвать функцию:</p>
<p>TWAIN_SelectImageSource(hwnd: HWND): Integer;</p>

<p>Данная функция позволяет выбрать источник получения данных из списка TWAIN совместимых устройств, в качестве параметра она получает хендл основного окна прикладной программы. Следует заменить, что если в системе имеется одно TWAIN совместимое устройство, то вызывать функцию не обязательно.</p>

<p>Для получения изображения служит функция:</p>
<p>TWAIN_AcquireNative(hwnd: HWND; pixmask: Integer): HBitmap;</p>

<p>где:</p>
<p>hwnd - хендел основного окна прикладной программы (допускается указывать 0);</p>
<p>pixmask - режим сканирования ( необходимо задавать 0 - указание другого режима может приводить к ошибке);</p>
<p>hBitmap - указатель на область памяти, содержащей полученные данные в DIB формате.</p>

<p>По окончании работы с DIB данными их необходимо удалить вызвав процедуру:</p>
<p>TWAIN_FreeNative(hDIB: HBitmap);</p>

<p>где:</p>
<p>hDIB - указатель, полученный при вызове функции TWAIN_AcquireNative.</p>

<p>Для облегчения обработки полученных DIB данных в библиотеке имеется несколько сервисных функций:</p>
<pre>
...
<p>Полный текст примера можно взять отсюда. Мы рассмотрим только функцию получения данных с TWAIN устройства:</p>
<pre>
