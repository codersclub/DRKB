<h1>Создание CGI счетчика в Delphi 5</h1>
...
<pre>       with Request.QueryFields do begin
...
<pre>         // Читаем старые значения счётчика
         AssignFile(fp,FileName);
         Reset(fp);
         ReadLn(fp,LastEver);
         Ever := LastEver+1;
...
<pre>         // Записываем новые значения счётчика
         AssignFile(fp,FileName);
         Rewrite(fp);
         WriteLn(fp,Ever);
         WriteLn(fp,Date);
