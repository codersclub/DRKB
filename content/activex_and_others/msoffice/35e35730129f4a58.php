...
&#8226; На главную форму выкладываем компоненту с закладки Servers, которая называется WordApplication<br>
&#8226; Устанавливаем свойства компоненты AutoConnect и AutoQuit в True<br>
&#8226; Запускаем приложение на выполнение.<br>
<p></p>
...
1. При создании формы, в системном реестре, по идентификатору CLSID был найден сервер Word.Application<br>
2. Запущено на выполнение приложение, находящееся по адресу в реестре (ProgID)<br>
3. Сервер предоставил нашему приложению, которое и является контроллером автоматизации интерфейс, через который мы и получим доступ к объекту Application.<br>
4. Интерфейс Idispatch унаследован от Iunknown, который в свою очередь имеет три метода, один из которых _ADDRef умеет считать количество клиентов, в настоящий момент использующих сервер. Как только от сервера отсоединиться последний клиент, он автоматически будет выгружен из памяти компьютера.<br>
<p>Закройте наше приложение и загляните в Task Manager - Word выгрузился из памяти.</p>
...
&#8226; Создайте новое приложение<br>
&#8226; Выложите на форму компонент WordApplication и WordDocument<br>
&#8226; Свойства AutoConnect и AutoQuit для WordApplication установите в True<br>
&#8226; Свойство ConnectKind для WordDocument установите в ckAttachToInterface<br>
&#8226; Для события onDokumentChange и onFormCreate пропишите следующий код:<br>
<pre class="delphi">
...
<p></p>
&#8226; Из среды программирования Delphi удаляем пакет с компонентами COM серверов (Component | Install Packages | Borland Sample Automation Servers Component | Remove). После этого закладка Servers удалилась.<br>
&#8226; Создаем новый пакет с использованием библиотеки типов<br>
&#8226; Выберите Project | Type library<br>
&#8226; Из списка зарегистрированных серверов выберите библиотеку типов Excel (Microsoft Excel 8.0 Object Library)<br>
&#8226; Укажите имя закладки палитры компонент (Pallete Page), куда будут установлены новые классы - TexcelQueryTable, TexcelApplication, TexcelChart, TexcelWorksheet, TexcelWorkbook, TExcelOLEObject . Выберите закладку Servers.<br>
&#8226; В боксе - General Component Wraper установите флажок для генерации компонеты на основе библиотеки типов и размещении ее на палитре компонент.<br>
&#8226; Нажмите кнопку Install<br>
&#8226; Специфицируйте имя пакета, где будет сгенерирован новый класс<br>
&#8226; Установите сервер на палитру компонент.<br>
<p>После проделанных манипуляций вам стал доступен COM сервер Excel, в состав которого входят шесть классов. Аналогичным образом восстановите компоненты для сервера Word и Outlook.</p>
<p>Теперь мы можем посмотреть на объявление класса TwordApplication и его предка - ToleServer:</p>
<pre class="delphi">TOleServer = class(TComponent, IUnknown)
...
&#8226; Передаваемые параметры, их типы и количество проверяется на стадии проектирования и редактор кода сопровождает разработчика всевозможными хинтами и подсказками.<br>
&#8226; Через Vtable интерфейс осуществляется значительно более быстрый доступ к серверу автоматизации, чем через DispInterface<br>
&#8226; В то же время, не всегда разработчик получает доступ к библиотеке типов и соответственно к V - таблице, поэтому приходится иногда пользоваться Idispatch интерфейсом<br>
<p>Ниже приведен пример использования Dual интерфейса</p>
<pre class="delphi">
procedure TForm1.FormCreate(Sender: TObject);
var
...
&#8226; Vtable<br>
&#8226; Idispatch<br>
&#8226; Позднее связывание (CreateOleObject)<br>
<p>Наиболее прогрессивный - первый способ, через который работают компоненты Delphi 5 для доступа к COM серверам приложений Office.</p>
...
begin
  ...
  begin
    //Подключаем объект класса TMailItem к новому елементу исходящих писем , 
    //для работы через Vtable интерфейс
    MailItem1.ConnectTo(_DMailItem(mapi.Folders.Item(olPersonal).
                      Folders.Item(olFolderOutbox).
                      Items.Add(olPostItem) as iDispatch));
