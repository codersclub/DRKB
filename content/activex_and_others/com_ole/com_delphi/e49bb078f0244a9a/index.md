...
![clip0059](/pic/clip0059.png){width="458" height="251"}
...
расположенного с ним в одной «комнате» клиента. Всю работу по
...
может иметь (или не иметь) свою STA       COINIT\_MULTITHREADED         
   Если в текущем процессе еще не создана MTA -- создается новая MTA,
...
![](/pic/embim1877.png){width="555" height="300"}

Параметр ThreadingModel может принимать следующие значения:

Apartment        Сервер может работать только в STA. Если он создается
из STA, то он будет создан в «комнате» вызывающего потока, если из MTA
-- COM автоматически создаст для него «комнату» c STA и proxy в
«комнате» клиента.       Free        Сервер может работать только в MTA.
...
MTA и proxy в «комнате» клиента.       Both        Сервер может работать
