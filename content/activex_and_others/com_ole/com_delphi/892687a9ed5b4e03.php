<h1>Введение и базовые понятия</h1>
...
<p>Интерфейс является неизменным контрактом.</p>
<p>Вы не должны определять новой версии того же интерфейса с измененным набором методов (или их параметров), но с тем же идентификатором. Это гарантирует, что новые интерфейсы никогда не конфликтуют со старыми. Если Вы нуждаетесь в расширении функциональности, Вы должны определить новый интерфейс, возможно являющийся наследником старого и реализовать дополнительные методы в нем.</p>
<p>Реализация интерфейса - это непосредственно код, который реализует эти методы. При этом за несколькими исключениями, не накладывается никаких ограничений на то, каким образом будет выглядеть реализация. Физически реализация представляет собой массив указателей на методы, адрес которого и используется в клиенте для доступа к COM-объекту. Любая реализация интерфейса имеет метод QueryInterface, позволяющий запросить ссылку на конкретный интерфейс из числа реализуемых.</p>

Автоматическое управление памятью и подсчет ссылок</p>
<p>Кроме предоставления независимого от языка программирования доступа к методам объектов COM реализует автоматическое управление памятью для COM-объектов. Оно основано на идее подсчета ссылок на объект. Любой клиент, желающий использовать COM объект после его создания должен вызвать заранее предопределенный метод, который увеличивает внутренний счетчик ссылок на объект на единицу. По завершении использования объекта клиент вызывает другой его метод, уменьшающий значение этого же счетчика. При достижении счетчиком ссылок нулевого значения COM-объект автоматически удаляет себя из памяти. Такая модель позволяет клиентам не вдаваться в подробности реализации объекта, а объекту - обслуживать несколько клиентов и корректно очистить память по завершении работы с последним из них.</p>

<p>Объявление интерфейсов</p>
<p>Для поддержки интерфейсов Delphi расширяет синтаксис языка Pascal дополнительными ключевыми словами. Объявление интерфейса в Delphi реализуется ключевым словом interface</p>
...
<p>Для генерации нового значения GUID в IDE Delphi служит сочетание клавиш Ctrl+Shift+G.</p>

<p>IUnknown</p>
<p>Базовым интерфейсом в модели COM является IUnknown. Любой интерфейс наследуется от IUnknown и обязан реализовать объявленные в нем методы. IUnknown объявлен в модуле System.pas следующим образом:</p>
...
<p>Рассмотрим назначение методов IUnknown более подробно.</p>
<p>Последние два метода предназначены для реализации механизма подсчета ссылок.</p>

<pre class="delphi">
function _AddRef: Integer; stdcall;
</pre>

<p>Эта функция должна увеличить счетчик ссылок на интерфейс на 1 и вернуть новое значение счетчика.</p>

<pre class="delphi">
function _Release: Integer; stdcall;
</pre>

<p>Эта функция должна уменьшить счетчик ссылок на интерфейс на 1 и вернуть новое значение счетчика. При достижении счетчиком значения 0 она должна освободить память, занятую реализацией интерфейса.</p>
<p>Первый метод позволяет получить ссылку на реализуемый классом интерфейс.</p>
...
- возвращает ссылку на него в параметрt Obj<br>
- вызывает метод _AddRef полученного интерфейса<br>
- возвращает 0<br>
В противном случае, функция возвращает код ошибки E_NOINTERFACE.</p>
...
- возвращает ссылку на него в параметр Obj<br>
- вызывает метод _AddRef полученного интерфейса<br>
- возвращает TRUE<br>
В противном случае, функция возвращает FALSE.</p>
