<h1>Расчет расстояния между 2-я точками на земной поверхности методом Винсенти</h1>
...
<hr /><p>Random <img src="/pic/embim1876.png" width="16" height="16" vspace="1" hspace="1" border="0" alt="">(08.09.2006 11:54) </p>
<p>недавно пришлось решать похожую задачку, однако приведенные вычисления слишком громоздкие, вот формула попроще:</p>
<p>&nbsp;<br>
distance=sqrt(pow((lon1 - lon2)*111*COS(lat2/57.295781), 2) + pow((lat1) - lat)*111, 2))<br>
&nbsp;<br>
проверялась по google maps (собственно для интеграции с ними и используется), расхождение составило не более 1,5%<br>
&nbsp;<br>
можно еще уточнить, если брать косинус не от одной широты а от их разности... но и так работает достаточно точно. Извиняюсь за недельфийский синтаксис (писал в mySQL), но думаю несложно будет перевести =). lat2/57.295781 - чтобы получить широту в радианах (опять же для mySQL).<br>
<p>Широта и долгота передается в формате google maps, то есть как вещественное число со знаком, размерность - градусы. Результат - в километрах.</p>
<hr />
<p>Попробуйте следующий код. Я им пользуюсь продолжительное время. </p>
<p>Входные данные: </p>
<p>StartLat (начальная широта) = Градусы и сотые доли </p>
<p>StartLong (начальная долгота) = Градусы и сотые доли </p>
<p>EndLat (конечная широта) = Градусы и сотые доли </p>
<p>EndLong (конечная долгота) = Градусы и сотые доли </p>
<p>Выходные данные: </p>
<p>Distance (расстояние) = Расстояние в метрах </p>
<p>Bearing (смещение) = Смещение в градусах </p>
<p>Не забудьте включить модуль Math в список используемых (USES) модулей. </p>
<pre>
var
