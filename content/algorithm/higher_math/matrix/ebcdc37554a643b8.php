<h1>Модуль реализации матричных вычислений для массивов больших размеров</h1>
<div class="date">01.01.2007</div>

<div class="author">Автор: Andrey, http://delphikingdom.com</div>

<p>В этом модуле &#171;осели&#187; все операции с матрицами и векторами, которые я использовал для работы. Но есть алгоритмы, которые многие, наверняка, увидят впервые: Divide - алгоритм прямого деления, MSqrt - квадратный корень, MAbs - абсолютная величина. Поскольку модуль содержит все, от элементарных операций до матричных, разобраться будет несложно:</p>
<p>Например, решение системы ЛУ (консольное приложение)</p>
<pre>
var
  N : Integer;
  A : Matrix;
...
<p>Некоторые алгоритмы требуют пояснения, например:</p>
<p>Matrix.E( i, j : LongWord ) или Vector.E( i : Integer ) : RealPtr</p>
<p>(RealPtr = ^Real) функция для вычисления адреса элемента матрицы/вектора. Перешла из ДОС когда в модуле использовался алгоритм управления виртуальной памятью для больших размерностей.</p>
<p>Matrix.Multiple( X, Y : Vector )</p>
<p>Результатом, которого является произведение вектора X на транспонированный вектор Y - матрица ранга 1.</p>
<p>Matrix.Invert( A : Matrix )</p>
<p>- если A[N,M], и N &lt;&gt; M то результат - матрица размера [M,N] - псевдообратная = A+.</p>
<p>Matrix.Addition( A : Matrix; B : Real )</p>
<p>- добавление числа в главную диагональ.</p>
<p>Matrix.Diag( r : Real )</p>
<p>- присваивание значения главной диагонали.</p>
...
<p>Этот модуль используется почти во всех реализованных мной численных алгоритмах и методах. Те части, которые писал не я - приводятся без изменений(по возможности) стиля и комментариев.</p>
<pre>
unit HNVString;
