<h1>Естественные ключи против искуственных ключей</h1>
<div class="date">20.07.1999</div>

<div class="author">Автор: Анатолий Тенцер</div>

<p>Статья 6-20 июля 1999 года, версия 1.1.</p>
...
<p>Есть два мнения:<br>
- СК должны использоваться, только если ЕК не существует. Если же ЕК существует, то идентификация записи внутри БД осуществляется по имеющемуся ЕК;<br>
- СК должны добавляться в любую таблицу, на которую существуют ссылки (REFERENCES) из других таблиц, и связи между ними должны организовываться только при помощи СК. Разумеется, поиск записи и представление её пользователю по прежнему производятся на основании ЕК.</td></tr></table></div>Естественно, можно представить себе и некое промежуточное мнение, но сейчас дискуссия ведётся в рамках двух вышеизложенных.
</p>

<p>Когда появляются СК</p>
...
<p>Вопрос достаточно спорный, однако, исходя из предположений, что:<br>
- База данных нормализована;<br>
- Записей в таблицах много (десятки тысяч и более);<br>
- Запросы преимущественно возвращают ограниченные наборы данных (максимум единицы процентов от размера таблицы).</td></tr></table></div>быстродействие системы на СК будет ощутимо выше. И вот почему:
</p>

<p>ЕК могут потенциально дать более высокое быстродействие, когда:<br>
- Требуется только информация, входящая в первичные ключи связанных таблиц;<br>
- нет условий WHERE по полям связанных таблиц.</p>

<p>Т.е., в нашем примере это запрос типа:</p>

<pre>SELECT Family, City FROM People;
</pre>

<p>В случае СК этот запрос будет выглядеть как</p>
<pre>SELECT P.Family, C.Name
...
<p>будет выполняться в разы медленнее, чем</p>
<pre>(CК) SELECT P.Family, C.Name</p>
...
<p>Увеличение скорости обновления данных</p>
...
<p>Типичные аргументы сторонников ЕК</p>
...
<p>В системе с ЕК меньше JOIN`ов, следовательно, запросы проще и разработка удобнее</p>
<p>Да, меньше. Hо, в системе с СК тривиально пишется:</p>

<pre>CREATE VIEW PeopleEK AS
...
<p>Введение ЕК нарушает третью нормальную форму</p>
