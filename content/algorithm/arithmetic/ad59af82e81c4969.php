...
<div class="author">Автор: Максим М. Гумеров</div>

<p>Возведение числа в действительную степень</p>
...
&#9642; - F2XM1 - вычисляет 2**x-1, где -1&lt;x&lt;1.<br>
&#9642; - FSCALE (масштабирование степенью двойки) - фактически считает 2**trunc(x), где trunc(x) означает округление к 0, т.е. положительные числа округляются в меньшую сторону, отрицательные - в большую.<br>
&#9642; - FXTRACT - извлекает мантиссу и экспоненту действительного числа.<br>
&#9642; - FYL2X - вычисляет y*log2(x), где log2(x) - логарифм x по основанию 2.<br>
...
<p>Вот, в общем-то, и все. Но уже на первый взгляд этого хватает, чтобы понять, что задача может быть решена более прямо, чем предлагает RTL Borland Delphi.</p>
...
<pre class="asm">; Во - первых, вычисляем z = y * log2(x):
fld y
...
<p>На вход функции Exp2 поступает единственный аргумент x - возводимое в степень число. Как можно реализовать функцию?</p>
...
Рисунок 1. Максимальная погрешность приближения функции Exp2=2**x (при x менее 990) не превышает 0,004%.</p>
...
<p>Аппроксимация функции log2x и “специализация” возведения в степень</p>
...
<p>ln(1+x)=x-x2/(1*2)+x3/(1*2*3)+…+ xi/i!+…</p>
...
<p>Таким образом, нам в этом случае (при x, близких к 1) удается избавиться от всех инструкций FPU, принадлежащих к группе трансцендентных, что приводит к впечатляющему росту производительности</p>
...
1. Intel® Architecture Software Developer&#8217;s Manual: том 2, Instruction set reference. Можно найти на сайте Intel www.intel.com.<br>
2. Intel® VTune™ Performance Analyzer, гипертекстовая справка. И вообще, VTune - замечательный инструмент для поиска шероховатостей в программе.<br>
3. Intel® Pentium® 4 and Intel® Xeon™ Processor Optimization Reference Manual. Все там же, на сайте Intel.<br>
