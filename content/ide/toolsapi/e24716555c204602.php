<h1>Еще примеры экспертов</h1>
<div class="date">01.01.2007</div>


<p>This article introduces you to the world of Delphi Experts. Delphi Experts are DLLs, that will be loaded during the startup sequence of Delphi. This article first appeared on Delphi-PRAXiShttp://www.delphipraxis.net/viewtopic.php?t=5300 in German.</p>

<p>NOTE: The techniques shown in this article are valid starting with Delphi 3 or 4 and since Delphi 7 they are deprecated, however, still fully suported by the Delphi IDE.</p>

<p class="p_Heading1">Installation of a Delphi-IDE-Expert</p>

<p>Every Delphi-Expert has to be registered in the Windows-Registry. For each Delphi-Version installed on a machine, as well as for each user using the machine, the Delphi-Expert has to be registered separately.</p>

<p>In the Registry the Delphi-Expert has to be registered under the folowing key:</p>

<p>HKCU\Software\Borland\Delphi\X.0\Experts</p>

<p>, where the X has to be replaced by the appropriate Delphi-Version supported. It may happen that the Experts key is not installed, in such case you are required to create it.</p>

<p>Underneath the Experts key you have to create a string value for the Delphi-Expert. The name must be unique. The value must point to the Delphi-Expert DLL, including both complete path and file name of the Delphi-Expert. Next time Delphi starts, the Expert will be loaded automatically.</p>

<p class="p_Heading1">The interface of the Delphi-Expert</p>

<p>In order for the Delphi Expert to interact with the Delphi-IDE ist has to export a function with the name ExpertEntryPoint, using the following parameters:</p>
...
<p>The first parameter ToolServices offers all "documented" interfaces to the Delphi-IDE. The second parameter RegisterProc is used to load the expert into the Delphi-IDE. The last parameter Teminate is used to notify the Expert-DLL when it is about to be unloaded by the Delphi-IDE.</p>

<p>The InitExpert method returns True, if the Expert has loaded successfully, otherwise it can eiter return False or raises an exception to unload the DLL from the Delphi-IDE (see code sample for solution).</p>

<p class="p_Heading1">The PlugIn class TIExpert</p>

<p>Any Delphi-Expert must be derived from the class TIExpert, which is declared in the unit ExptIntf. This class defines some abstract methods, which must be implemented by each PlugIn: GetName, GetAuthor, GetComment, GetPage, GetGlyph (different for Windows and Linux), GetStyle, GetState, GetIDString, GetMenuText and Execute. The purpose of each method is explained in the source code below.</p>

<p class="p_Heading1">The simplest Delphi-Expert</p>

<p>This Delphi-Expert want do much good, however, it shows you the basic way of getting the job done. It will show an entry in the Help menu (default behavior). Once the user clicks the menu item the method Execute from the Expert will be called. The following points must be respected in order to get the expert working:</p>

&#183; The method GetState must return [esEnabled]<br />
&#183; The method GetStyle must return esStandard<br />
&#183; The method GetMenuText returns the text to be shown in the Help menu<br />
&#183; The method Execute defines the expert action upon activation<br />
<br />
<p>The Library Source Code (DelphiPlugI.dpr)</p>
