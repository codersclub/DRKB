<h1>Перенос приложений с BDE на dbExpress (статья)</h1>
<div class="date">01.11.2002</div>

<div class="author">Bill Todd, The Database Group, Inc.</div>

<p>for Borland Software Corporation</p>
<p>September 2002</p>

<div class="author">Перевод: by KDV (www.ibase.ru), ноябрь 2002. С разрешения Borland.</div>
<p>оригинал статьи: http://community.borland.com/article/images/29106/migrating_bde_applications_to_dbexpress.pdf</p>

<p>Содержание</p>
1. Borland dbExpress<br>
2. Архитектура dbExpress<br>
3. Как работает архитектура provider/resolver<br>
4. Создание приложений на dbExpress<br>
5. BDE против dbExpress<br>
6. Перенос клиент-серверного приложения BDE на dbExpress<br>
7. Перенос локальных приложений BDE на dbExpress<br>
8. Итог<br>
9. Об авторе<br>
</p>
...
<p>
&#8226; минимизировать объем и количество используемых ресурсов<br>
&#8226; получить максимальную скорость работы<br>
&#8226; обеспечить кросс-платформенность<br>
&#8226; обеспечить легкость распространения<br>
&#8226; обеспечить легкость разработки драйверов<br>
...
<p>
&#8226; вложенность таблицы деталей в поле главной таблицы<br>
&#8226; редактирование данных через SQLQuery, DataSetProvider, и ClientDataSet<br>
&#8226; использование SQLQuery как источника данных только для чтения без DataSetProvider и ClientDataSet<br>
&#8226; применение изменений, накопленных ClientDataSet, к базе данных<br>
...
<p>
1. Создайте новое приложение и добавим data module. Назовем модуль данных MainDm<br>
2. Используйте окно Project Options, для того чтобы убедиться, что модуль создается автоматически перед созданием главной формы<br>
3. Поместите компонент SQLConnection с закладки dbExpress на модуль данных<br>
4. Назовите компонент SQLConnection как EmployeeConnection и установите его свойство DriverName в Interbase<br>
5. Откройте редактор свойств Params и установим параметр Database в путь к EMPLOYEE.GDB<br>
6. Измените значения свойств UserName и Password на нужные<br>
7. Установите свойство LoginPrompt в False, чтобы не появлялось окно запроса имени пользователя и пароля при старте программы<br>
8. Установите свойство Connected в True для проверки соединения, и обратно в False
<p>

<p>Компонент SQLConnection обеспечивает соединение с базой данных для любого количества компонент. Вы можете использовать несколько SQLConnection для подсоединения к нескольким базам данных одновременно.</p>
...
<p>
1. Поместите три компонента SQLQuery на модуль данных<br>
2. установите их свойство SQLConnection в EmployeeConnection<br>
3. Назовите первый компонент EmployeeQry, второй HistoryQry, и третий - DeptQry<br>
...
<p>5. Установите свойство SQL компонента HistoryQry в</p>
...
<p>6. Установите свойство SQL компонента DeptQry в</p>
...
8. Установите свойство компонента HistoryQry.DataSource в EmpLinkSrc<br>
9. Вернитесь к EmployeeQry, откройте редактор свойства Params, и установите значение параметра dept_no в XXX. Поскольку это неверное значение, в результате не будет отображено ни одной записи, пока пользователь не введет правильный номер отдела.<br>
10. Сделайте двойной клик на EmployeeQry, для того чтобы открыть Fields Editor. Добавьте все поля (в меню по правой кнопке add all fields)<br>
11. Выберите столбец emp_no, свойство ProviderFlags, и установите pfInKey в True. Установка этого флага указывает на то, что столбец emp_no является первичным ключом. Компоненту DataSetProvider (который мы добавим позже) нужна эта информация для генерации операторов SQL, модифицирующих данные.<br>
12. Выберите столбец full_name, свойство ProviderFlags, и установите pfInUpdate и pfInWhere в False. Столбец full_name является вычисляемым полем, поэтому не может быть обновлен и не нужен в условии where SQL операторов, генерируемых DataSetProvider.<br>
13. Установите свойство Active компонента EmployeeQry в True<br>
14. Сделайте двойной клик на HistoryQry, и в FieldEditor добавьте все поля (add all fields)<br>
15. Выберите поле emp_no и установите pfInKey в True. Повторите то же самое для полей change_date и updater_id, т.к. все эти три поля являются первичным ключом.<br>
16. Поле new_salary также является вычисляемым, поэтому для него тоже нужно установить pfInUpdate и pfInWhere в False<br>
17. Установите свойство Active компонента EmployeeQry в False<br>
18. Сделайте двойной клик на компоненте DeptQry, добавьте все поля (add all fields)<br>
...
<p>
&#8226; вы не можете использовать его в многозвенных приложениях. Если в дальнейшем потребуется конвертировать приложение в многозвенное, используйте отдельные компоненты<br>
&#8226; невозможно подсоединить дочерний DataSet<br>
&#8226; События встроенного DataSetProvider не экспортированы<br>
&#8226; Свойство Options встроенного DataSetProvider не экспортировано. Вы не можете устанавливать параметры провайдера во время разработки или выполнения<br>
&#8226; Вы не можете определять поля встроенного DataSet в момент разработки. Это означает, что вы не можете установить свойство ProviderFlags во время разработки. Вам придется делать это в коде приложения<br>
&#8226; Если вы используете Borland MyBase (база данных встроенная в ClientDataSet), и не работаете с сервером баз данных, то лучше использовать ClientDataSet отдельно, для уменьшения используемых ресурсов<br>
...
<p>
1. Добавьте DataSetProvider с палитры DataAccess на модуль данных<br>
...
<p>
1. Поместите два компонента ClientDataSet на модуль данных<br>
2. установите свойство ProviderName первого компонента на EmployeeProv, и назовите его EmployeeCds<br>
3. сделайте двойной клик на EmployeeCds, чтобы открыть FieldEditor. Выполните Add all fields. Последнее поле имеет имя HistoryQry. Это поле вложенного DataSet, которое содержит историю зарплат (salary history) для каждой записи о сотруднике (employee). Записи, возвращаемые HistoryQry видны как вложенные данные внутри данных EmployeeQry. Это происходит потому, что свойство HistoryQry.DataSource установлено в EmpLinkSrc, который подсоединен к EmployeeQry, и получает значение параметра запроса из записи EmployeeQry.<br>
4. Щелкните правой кнопкой мыши на EmployeeCds и выберите пункт меню FetchParams, чтобы ClientDataSet обновил список параметров в соответствии с компонентом EmployeeQry.<br>
5. Назовите второй ClientDataSet как HistoryCds и установите его свойство DataSetField в EmploueeCdsHistoryQry. Теперь HistoryCds будет получать данные из поля вложенного набора данных EmployeeCds<br>
6. Сделайте двойной клик на HistoryCds, и в Field Editor добавьте все поля (add all fields)<br>
7. Поместите два компонента DataSource на модуль данных. Назовите их EmployeeSrc и HistorySrc.<br>
8. Установите свойство DataSet у этих компонент - у EmployeeSrc в EmployeeCds, и у HistorySrc в HistoryCds.
</p>

<p>Теперь можно установить свойство Active в True у компонент EmployeeCds и HistoryCds. Модуль данных при этом будет выглядеть следующим образом:</p>
...
<p>
1. Добавьте в проект Reconcile Error Dialog с закладки Dialogs репозитария объектов<br>
2. Добавьте имя нового модуля в оператор USES модуля данных.<br>
3. В свойствах проекта убедитесь, что модуль Reconcile Error Dialog не создается автоматически при запуске приложения<br>
4. Создайте обработчик OnReconcile для EmployeeCds, и добавьте туда строку
</p>

<pre>Action := HandleReconcileError(DataSet, UpdateKind, E);</pre>
...
<p>
1. Добавьте два компонента Panel, два DBGrid и два DBNavigator на главную форму приложения, так как это показано на картинке<br>
2. Добавьте имя модуля данных в USES главной формы<br>
3. Установите свойство DataSource верхнего DBGrid и DBNavigator в EmployeeSrc<br>
4. Установите свойство DataSource нижнего DBGrid и DBNavigator в HistorySrc<br>
5. Добавьте Label и ComboBox<br>
6. Создайте событие OnChange для ComboBox со следующим текстом<br>
...
<p>Добавьте кнопку (Button) на верхнюю панель, и установите ее надпись в "Сохранить".<br>
...
<p>Добавьте метод к форме MainForm для заполнения списка выбора номеров отделов</p>
...
<p>Добавьте обработчик OnCreate для главной формы</p>
...
<tr>
  <td>буферизация записей</td>
  <td>BDE сам определяет количество записей, хранимых в памяти</td>
  <td>Вы можете управлять количеством записей, хранимых в памяти</td>
</tr>
<tr>
  <td>контроль сетевого трафика</td>
  <td>BDE сам определяет, сколько записей выбирать с сервера</td>
  <td>Вы управляете как количеством, так и моментом выборки записей с сервера</td>
</tr>
<tr>
  <td>управление транзакциями</td>
  <td>Есть автоматический режим и ручное управление транзакциями.
      Транзакции активны в момент редактирования данных.</td>
  <td>Есть автоматический режим и ручное управление транзакциями.
      Транзакции активны в момент передачи изменений на сервер.</td>
</tr>
<tr>
  <td>распространение приложений</td>
  <td>Большое количество файлов (объемом около 9 мегабайт).
      Установка и конфигурирование достаточно сложны и требуют редактирования registry.</td>
  <td>Для распространения нужно две dll, занимающие менее 500 килобайт,
      которые могут быть вкомпилированы в EXE.
      Конфигурация хранится в dbxconnections.ini и dbxdrivers.ini</td>
</tr>
<tr>
  <td>кросс-платформенность</td>
  <td>Только Windows</td>
  <td>Windows и Linux</td>
</tr>
<tr>
  <td>Драйверы от третьих фирм</td>
  <td>Сложно разрабатывать, практически нет</td>
  <td>Легко разрабатывать. Доступно много разных драйверов.
      За информацией по драйверам обращайтесь на bdn.borland.com</td>
</tr>
</table>
...
<table>
<tr><td>BDE                </td><td>dbExpress</td></tr>
<tr><td>TDatabase          </td><td>TSQLConnection</td></tr>
<tr><td>TQuery             </td><td>TSQLQuery</td></tr>
<tr><td>TStoredProc        </td><td>TSQLStoredProc</td></tr>
<tr><td>TTable             </td><td>TSQLTable</td></tr>
<tr><td>нет аналога        </td><td>TSQLDataSet</td></tr>
<tr><td>TBatchMove         </td><td>нет аналога</td></tr>
<tr><td>утилита SQL Monitor</td><td>TSQLMonitor</td></tr>
<tr><td>TSession           </td><td>нет</td></tr>
<tr><td>TUpdateSQL         </td><td>нет</td></tr>
<tr><td>NestedDataSet      </td><td>нет</td></tr>
<tr><td>BDEClientDataSet   </td><td>SimpleDataSet</td></tr>
</table>
...
&#8226; Замените компонент TDatabase на компонент SQLConnection<br>
&#8226; Замените компоненты TQuery и TStoredProc на компоненты SQLQuery и SQLStoredProc<br>
...
&#8226; Вы должны добавить вызовы ApplyUpdates перед каждым Commit (и CancelUpdates при Rollback)<br>
&#8226; Вы должны перенести место вызова StartTransaction если хотите получить преимущества коротких транзакций (возможно, перед соответствующими ApplyUpdates)<br>
&#8226; При каждом вызове StartTransaction, Commit и Rollback нужно передавать параметр TTransactionDesc<br>
<p>Как видите, меньше работы при удалении кода, явно управляющего транзакциями, чем при его модификации. Давайте посмотрим на эти три пункта подробнее.</p>
...
<p>В клиент-серверных приложениях BDE управление транзакциями выглядит следующим образом:<br>
1. Старт транзакции<br>
2. Редактирование данных пользователем<br>
3. Commit или Rollback<br>
</p>

<p>В этой модели транзакция активна все время, пока пользователь редактирует данные. Одно из главных достоинств dbExpress это короткое время жизни транзакций. Для использования этого достоинства управление транзакциями должно быть изменено на следующую схему:</p>
1. Пользователь редактирует данные в ClientDataSet<br>
2. Старт транзакции<br>
3. Вызов ApplyUpdates<br>
4. Commit или Rollback<br>
<p>Для изменения приложения в соответствии с этой схемой, нужно переместить каждый вызов StartTransaction в место, где будет выполняться соответствующий Commit. Затем между ними вставить вызов ApplyUpdates.</p>
...
&#8226; Используйте утилиту DataPump, входящую в поставку Delphi или C++Builder<br>
&#8226; Используйте утилиты переноса данных третьих фирм (kdv - например IBDataPump)<br>
&#8226; Напишите свою утилиту, переносящую данные из одного формата в другой<br>
<p>Используемый SQL сервер может не поддерживать все типы данных Paradox или dBase. Например, в Paradox есть тип boolean, который есть не во всех SQL-серверах (kdv - boolean есть в Interbase 7). Также нужно убедиться, что утилита копирования данных правильно соотносит типы данных источника и приемника.</p>
...
1. заменить каждый Table компонентами SQLTable, DataSetProvider и ClientDataSet<br>
2. заменить каждый Table компонентами SQLQuery, DataSetProvider и ClientDataSet. Выбирать все столбцы и все записи из таблицы.<br>
3. заменить каждый Table компонентами SQLQuery, DataSetProvider и ClientDataSet. Переписать приложение для работы в client-server.<br>
<p>Вариант 1 является наиболее простым, т.к. свойства SQLTable почти целиком повторяют свойства Table. Если таблицы не очень большие, то производительность будет достаточной. Недостаток этого варианта в том, что если вы захотите выбирать часть данных с сервера вместо всех данных, вам придется заменить компонент SQLTable на SQLQuery.</p>
...
&#8226; более короткие транзакции<br>
&#8226; больше контроля над сетевым трафиком и использованием ресурсов<br>
&#8226; уменьшение размера программы и используемых ею ресурсов<br>
&#8226; повышенная производительность<br>
&#8226; легкое распространение приложения<br>
&#8226; небольшой набор файлов для распространения<br>
&#8226; кросс-платформенная разработка (Windows и Linux)<br>

<p>dbExpress минимизирует усилия по переносу приложений, т.к. он включает набор компонент, содержащих свойства и события, похожие на компоненты BDE настолько, насколько это возможно. Это позволяет вам перейти на качественную технологию работы с данными при минимуме изменения исходного кода ваших приложений.</p>
