<h1>Архитектура BDE и его особенности при работе с SQL-серверами (статья)</h1>
<div class="date">17.08.2001</div>

<div class="author">Автор: KDV, www.ibase.ru</div>

<p>Как работает BDE</p>
<p>или</p>
<p>Архитектура BDE и его особенности при работе с SQL-серверами</p>
<p>Автор: Кузьменко Дмитрий, www.ibase.ru. 25.08.2000 (исправления от 14.12.2000, 17.08.2001).</p>
...
<p class="note">Примечание: дальнейшая информация по датам взята из документа, подзаголовок "Evolution of BDE/IDAPI Technology: 1990 - 94".</p>
...
<p>
* IDAPI32.DLL - основной интерфейс<br>
* BLW32.DLL, BANTAM.DLL- языковые функции<br>
* *.BTL - файлы с языковыми кодировками.<br>
* IDBAT32.DLL - операции пакетного копирования данных<br>
* IDDR32.DLL - модуль работы с Data Repository<br>
* IDASCI32.DLL - драйвер для работы с текстовым форматом<br>
* IDDAO32.DLL - драйвер трансляции вызовов к DAO<br>
* IDODBC32.DLL - драйвер трансляции вызовов к ODBC<br>
* IDPDX32.DLL - драйвер для работы с форматом Paradox<br>
* IDDBAS32.DLL - драйвер для работы с форматом dBase и FoxPro<br>
* IDQBE32.DLL - ядро обработки запросов QBE<br>
* IDSQL32.DLL - ядро обработки запросов SQL<br>
* SQLINT32.DLL - SQLLink-драйвер трансляции вызовов к Interbase API<br>
* SQLORA32.DLL - SQLLink-драйвер трансляции вызовов к Oracle Call Level Interface<br>
* SQL*32.DLL - другие SQLLink-драйверы
</p>

<p>Таким образом, при установке BDE "лишние" файлы можно без проблем выкинуть.</p>
...
<p class="note">Примечание: можно было бы отнести этот недостаток на SQL Link для IB, но вполне возможно что просто TTable не в состоянии кэшировать и обновлять данные, отсортированные по убыванию (см. дальше о кэше данных).</p>
...
<p class="note">Примечание: вы сами можете убедиться в изложенном, поместив первый запрос в TQuery, переключив RequestLive в True. Попытайтесь установить Active компонента в True и посмотрите что получится.</p>
...
<p class="note">Примечание: Даже если компонент IBX IBTable и поддерживает Refresh, то он его выполняют точно таким же образом, что и BDE. А компонент IBDataSet выполняет Refresh только для одной, текущей, записи.</p>
...
<p>
* открытый query довыбирает данные.<br>
* открытый table закрывается
</p>

<p>другие случаи я не упомянул, потому что IB SQL Link их не поддерживает. То есть при любом завершении транзакции (и открытии новой) данные будут перечитываться. Для TTable это не смертельно, т.к. он знает первичный ключ записи, на которой стоял курсор грида, и может перечитать немного данных, чтобы заново отобразить их. А вот для TQuery, который не знает никаких первичных ключей, происходит полная выборка всех данных, что эквивалентно вызову Locate, FetchAll или Last. Так что если ваше приложение при обновлении данных почему-то сильно тормозит, или возникают паузы, то нужно срочно смотреть в SQL Monitor, какие именно запросы перечитываются.</p>
...
<p>
* upWhereAll, по умолчанию - BDE пытается сделать UPDATE с внесением в условие WHERE всех значений полей, которые были ДО момента редактирования. Если при этом произошла ошибка, значит хотя бы одно поле у редактируемой записи уже было кем-то изменено (с момента входа в режим редактирования до момента Post).<br>
* upWhereChanged - BDE пытается сделать UPDATE с условием WHERE, проверяющим старые значения только измененных полей. Т.е. чтобы убедиться, что пользователь поменял именно те значения полей, которые видел, на новые. Если произошла ошибка, то это значит что одно из изменяемых полей было уже кем-то изменено.<br>
* upWhereKeyOnly - BDE обновляет запись, устанавливая в WHERE поиск записи только по ее первичному ключу.
</p>

<p>Соответственно, если запись не найдена, то выдается упомянутое в заголовке сообщение Record/Key deleted.</p>

<p class="note">Примечание: найдена запись или нет, определяется анализом переменной RowsAffected, куда заносится информация после выполнения запросов UPDATE/DELETE. Если RowsAffected = 0, то Record/Key Deleted.</p>
...
<p>
1. открыть 2 или более TDatabase, каждый для соответствующей базы данных. Например, один компонент подсоединен как A к алиасу TEST, а другой, как<br>
2. открыть TDatabase, который подсоединен к драйверу типа STANDARD(т.е. к локальным таблицам. Существование оных необязательно). См. окно свойств TDatabase.<br>
...
<p>
1. ядро Local SQL "разбирает" запрос, и выясняет, какие таблицы из каких баз данных используются в запросе<br>
2. данные из каждой таблицы вытаскиваются в локальный кэш (т.е. на клиента), в память или временные таблицы.<br>
