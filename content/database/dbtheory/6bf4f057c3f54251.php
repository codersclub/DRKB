<h1>Архитектура «Клиент-cервер»</h1>
...
<img src="/pic/embim1707.png" width="507" height="197" alt=""></p>
...
1. Выполнять клиентские запросы по извлечению и модификации данных;<br>
2. Обеспечивать одновременный доступ к данным нескольких пользователей;<br>
3. Обеспечивать идентификацию пользователей и разграничение прав доступа разных пользователей к разным данным;<br>
4. Обеспечивать целостность и непротиворечивость данных в случае аппаратных и программных сбоев;<br>
5. Защищать данные от несакционированного доступа;<br>
6. Предоставлять дополнительные средства администрирования информационной системы.<br>
<p>
<p>Рассмотрим эти требования более подробно.</p>

<p>Выполнять клиентские запросы по извлечению и модификации данных. Вообще говоря, это основная функция сервера баз данных. Механизм реализации этой функции может быть скрыт от пользователя, то есть пользователь (а, точнее, работающая с ним программаклиент) просто формулирует ЧТО ему нужно, а сервер базы данных исполняет этот запрос. В зависимости от того, какие функции предоставляет сервер базы данных, какие модели данных можно иметь на сервере, различают различные типы СУБД. Более подробно о том, какие бывают СУБД, будет говориться в отдельной главе.</p>
...
<tr>
<td></td>
<td>Программаклиент</td>
<td>Сервер базы данных</td>
</tr>

<tr>
<td>1</td>
<td>Оператор запускает клиента, клиент посылает запрос на установление связи.</td>
<td></td>
</tr>
...
<td></td>
<td></td>
<td>Сервер посылает клиенту подтверждение, о том, что он готов к работе.</td>
</tr>

<tr>
<td>2</td>
<td>Клиент запрашивает регистрационное имя и пароль пользователя. После ввода этих данных, имя и пароль посылаются на сервер.</td>
<td></td>
</tr>
...
<td></td>
<td></td>
<td>Сервер проверяет, соответствует ли пароль имени пользователя, и имеет ли право данный пользователь работать с нужной базой данных. Если все права соблюдены, то клиенту отсылается подтверждение.</td>
</tr>

<tr>
<td>3</td>
<td>Пользователь заполняет форму ввода, где указывает полученную по телефону информацию. Информация из формы ввода передается на сервер с запросом на ее внесение в базу.</td>
<td></td>
</tr>
...
<td></td>
<td></td>
<td>Сервер проверяет, имеет ли право данный пользователь вносить новую информацию. Если да, то сервер проверяет не будет ли нарушена целостность данных. Если все нормально, то производится вставка новых данных и клиенту пересылается уведомление о том, что все прошло успешно.</td>
</tr>

<tr>
<td>4</td>
<td>Клиент сообщает пользователю об успешной операции.</td>
<td></td>
</tr>

<tr>
<td>5</td>
<td>Пункты 3 и 4 повторяются...</td>
<td></td>
</tr>

</table>
<p>Рис. 1.4. Пример взамодействие клиента и сервера</p>

<p>Конечно же, в реальной ситуации взаимодействие между сервером базы данных и программойклиентом может быть значительно сложнее. Да и действия, производимые сервером базы данных, также включают в себя дополнительные проверки и действия. Однако, в данном примере показаны реальные принципы и схемы взаимодействия между клиентом и сервером.</p>
