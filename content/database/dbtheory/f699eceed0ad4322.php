...
<p>Уровни абстракции данных. Физический, логический, концептуальный уровни. Модели данных. Основные идеи иерархической, сетевой, реляционной и объектно-реляционной моделей.</p>

<p>2.1. Уровни восприятия данных</p>
...
<p>Рис. 2.1. Уровни восприятия базы данных.</p>

<p>Реализацией физического уровня, как правило, занимаются специализированные фирмы, такие как Informix Software, Software AG, CA, Oracle Corporation и т.д. Реализацией концептуального и пользовательского уровней занимаются либо программисткие отделы предприятий, либо специализированные фирмы.</p>
...
<p>2.2. Иерархическая модель данных</p>
...
<p><img src="/pic/embim1709.png" width="552" height="312" vspace="1" hspace="1" border="0" alt=""></p>
Рис. 2.2. Графическое представление иерархической модели данных (справа пример какой-то конкретной базы данных).</p>

<p>Что касается способов описания конкретных схем, базирующихся на иерархической модели, или языков манипулирования данными, работающими с такой моделью, то они зависят от конкретной реализации. Например, в достаточно давно разработанной системе IMS фирмы IBM (эта система является классическим примером иерархической СУБД) схема для нашего примера будет описана следующим образом):</p>
...
<p>2.4. Реляционная модель данных</p>
...
<p>Теперь по названию сборника всегда можно определить название и адрес издательства (или нескольких издательств), его выпустившего. Для этого надо в таблице "Сборники" найти запись, у которой поле "Сборник" есть требуемое название, из найденной запись взять название издательства (атрибут “Издательство”), а потом в таблице "Издательства" найти нужный адрес.</p>
...
<p>Запрос, который определит адрес издательства (или издательств), выпустивший сборник “Китайские воздушные змеи” будет выглядеть следующим образом:</p>
<pre>
...
<p>2.5. Объектно-реляционная модель данных</p>
...
- определение новых базовых классов(типов);<br>
- определение новых составных классов (типов) на базе существующих;<br>
- введение новых функций, работающих как с предопределенными типами и классами данных, так и с новыми;<br>
- наследование на уровне классов;<br>
- наследование на уровне таблиц;<br>
- обеспечение инкапсуляции для типов;<br>

<p>Так как ОРСУБД поддерживают стандартный SQL, то построение схемы базы данных для нашего примера с издательствами в объектно-реляционной СУБД будет выглядить точно так же, как и для реляционных СУБД. Если же  мы захотим воспользоваться преимуществами объектно-реляционной модели, в нашем примере мы можем определить новый тип данных “Адрес”:</p>
...
<p>и создать таблицу “Издательства” с использованием этого нового типа:</p>
<pre>
CREATE TABLE Издательства (
        Название      CHAR(20),
        Адрес         адрес_т
...
<p>Далее, при описании каждой из статей надо хранить список ее авторов. В реляционной модели это делать не очень удобно, зато объектно-реляционная СУБД позволит сделать это естественным и элегантным путем - для каждой статьи заводится атрибут “Авторы”, типа множество значений типа Автор_т:</p>
<pre>
CREATE TABLE Статьи (
        Название      CHAR(20),
        Авторы        SET (Автор_т)
