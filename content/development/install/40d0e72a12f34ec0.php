...
<p>Запомните одно важное правило: инсталлировать программу можно с человеческих носителей
(винчестеры, компакт-диски, ZIP-диски) и с дискет :) Если вы собираетесь написать инсталляцию
с дискет, которая явно не поместиться на одну дискету, то у вас есть шанс хорошо провести время :)</p>

<p>Как вы знаете, Windows сбрасывает ненужную ей в данный момент информацию на диск.
...
Когда ваша программа снова становиться нужна, Windows снова загружает её из выполняемого файла.</p>

<p>Эта в высшей степени корректная техника перестаёт работать при инсталляции с дискет.
...
и естественно его не находит (на четвёртой-то дискете? откуда?).</p>

<p>Только не паникуйте! Эта проблема давно решена, иначе вы не могли бы установить
...
если вы инсталлируетесь не с дискет, или если ваша инсталляция умещается на одну дискету.</p>

<p>На втором этапе программа инсталляции обычно показывает пользователю несколько страшных
предупреждений; что-то типа "если вы не заплатите за эту программу, то сидеть вам в тюрьме три пожизненных срока"
Я слышал, что некоторые пользователи со слабым сердцем даже умирали за компьютером от таких угроз :)</p>

<p>Реализация этого этапа до идиотизма тривиальна, поэтому мы и не будем на нём останавливаться подробно.</p>

<p>Следущий этап &#8212; третий. Здесь программа установки дотошно выспрашивает у пользователя кучу
...
трудности, но их несложно обойти.</p>

<p>Четвёртый этап &#8212; копирование. Конечно, это не очень сложно, но некоторые проблемы у нас всё-таки возникнут.
...
Вы ещё не передумали писать программу инсталляции?</p>

<p>Следующий, пятый, этап &#8212; настройка системного реестра (registry).
Достаточно тривиальная процедура, правда, при инсталляции большого продукта, записывать придёться очень много.</p>

<p>Предпоследний, шестой, этап, заключается в создании группы программ в меню "Пуск".
Или, возможно, вы захотите вынести ярлык на рабочий стол.</p>

<p>Наконец, финальная часть включает демонстрацию нескольких файлов (например, readme),
затем онлайновую регистрацию (подробно на ней я останавливаться не буду) и последнее
сообщение "Инсталляция успешно завершена".</p>
...
<p>Есть две грабли, на которые можно наступить в приведённом примере.
Первые лежат в вызове функции GetTempPath.
Если у вас нет переменных окружения TMP и TEMP,
то временным каталогом станет текущий каталог программы, то есть, фактически, ваша дискета.</p>
...
<p class="note">Примечание: Если для вас важен размер вашей инсталляции, вы можете взять только тот кусочек,</p>
...
<p>В доступной мне версии Windows 95, эти значения хранятся в ветке
...
Версию операционной системы можно узнать с помощью функции GetVersionEx.</p>

<p>Куда копировать программу:</p>

<p>Можно сформулировать наш вопрос и по другому: где находится каталог Program Files?
Некоторые инсталляции считают, что это C:\Program Files. В действительности, конечно,
он может находиться на другом диске, поэтому мы попробуем поискать его по другому... в реестре.</p>

<pre>
HKEY_LOCAL_MACHINE\Software\Microsoft\Windows\CurrentVersion\ ProgramFilesDir = 'D:\Program Files'
</pre>

<p>Можно воспользоваться функцией SHGetSpecialFolderLocation (это даже более корректно с точки зрения Microsoft).
...
Подробнее о выборе каталога мы поговорим позднее, когда будем рассматривать тонкости процесса инсталляции.</p>
