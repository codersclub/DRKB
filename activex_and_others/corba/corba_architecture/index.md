...
не влияющих на определение интерфейса для данного объекта. Брокер
Объектных Запросов.
...
Брокеров Объектных Запросов. Объекты.

Система объектов обеспечивает клиента набором сервисов. Клиент способен
запросить некоторый сервис. Объект - это нечто, что обеспечивает один
или более сервисов, которые клиент может запросить. Пример Брокеров
Объектных Запросов.
...
конкретный ORB может быть реализован сразу несколькими способами. ORB,
включаемый в клиентское и серверное приложение.
...
нормальных механизмов IPC. ORB как часть системы.
...
данных, если клиент и сервер находятся на одной и той же машине. ORB,
основанный на библиотеках.
...
разрушит эти данные. Реализации объектов.
...
дальнейшее функционирование ORB-а. Скелет реализации.
...
ORB вызывает требуемые процедуры. Динамическая обработка запросов.
...
процедур-заглушек, каждый метод дает одинаковый результат. Запросы.
...
параметры, специфические для данного типа исключений. Параметры.Параметр
характеризуется режимом передачи и своим типом. Режим определяет, должно
...
реализацией объекта. Интерфейс динамического выполнения вызовов.
...
помощью хранилища описаний объектов. Клиентская часть - заглушки.
...
Smalltalk такого интерфейса не требуют. Обзор протокола GIOP.
...
::: {style="text-align: left; text-indent: 0px; padding: 0px 0px 0px 0px; margin: 7px 0px 7px 24px;"}
  ---- --------------------------------------------------------------------------------------------------------------------------------------------------------------------
  1.   Формат сообщения протокола GIOP. Сообщения протокола GIOP обеспечивают нахождение объекта, отработку запросов, а также простейшее управление каналом коммуникации.
  ---- --------------------------------------------------------------------------------------------------------------------------------------------------------------------
:::

::: {style="text-align: left; text-indent: 0px; padding: 0px 0px 0px 0px; margin: 7px 0px 7px 24px;"}
  ---- -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  1.   Предположения о транспорте. Спецификация GIOP описывает общие предположения, которые делаются при рассмотрении любого сетевого транспортного слоя, который может быть использован для обмена сообщениями протокола GIOP. Также описываются общие принципы управления соединением.
  ---- -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
:::
...
производителей. Особенности и цели протокола.
...
программно-зависимых аспектов. При разработке этих протоколов
преследовались следующие цели: Распространенность.
...
между отдельными ORB-ами. Простота.
...
протоколу практически в любой системе. Масштабируемость.
...
распространения готовых ORB-ов. Общность.
...
на установленном между сервером и клиентом соединении. Архитектурная
независимость.
...
принимать и отправлять сообщения по протоколу IIOP. Формат сообщений
протокола GIOP.
...
Заголовок сообщения однозначно определяет его тип. Заголовок определен
...
::: {style="text-align: left; text-indent: 0px; padding: 0px 0px 0px 0px; margin: 7px 0px 7px 24px;"}
  ---- ----------------------------------------------------------------------------------------------------------------
  1.   Транспорт протокол должен гарантировать прохождение переданных байт в том порядке, в котором они были посланы.
  ---- ----------------------------------------------------------------------------------------------------------------
:::

::: {style="text-align: left; text-indent: 0px; padding: 0px 0px 0px 0px; margin: 7px 0px 7px 24px;"}
  ---- -------------------------------------------------------------------------------------------------------------------------------------------
  1.   Транспорт может рассматриваться как поток байт без дополнительных ограничений на размеры, фрагментацию или выравнивание размеров посылок.
  ---- -------------------------------------------------------------------------------------------------------------------------------------------
:::

::: {style="text-align: left; text-indent: 0px; padding: 0px 0px 0px 0px; margin: 7px 0px 7px 24px;"}
  ---- ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  1.   Транспорт должен обеспечивать сигнализацию об разрыве соединения. Если один из участников обмена неожиданно прервал свою работу, произошел сбой в операционной системе или сети, то другой должен быть уведомлен об этом.
  ---- ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
:::
...
описанной транспортной модели. Управление соединением.
...
Каждое сообщение типа Request должно иметь уникальный номер, который
...
таких случаев. Язык описания интерфейсов.
...
программирования. Отображение IDL в языки программирования.
...
Отображение для языка также определяет порядок взаимодействия между
...
с вызовом метода объекта. Типы данных.
...
рисунке 2-1. Синтаксис Общего Представления Данных - CDR.
...
Протокол GIOP определяет два вида потоков - сообщение и инкапсуляция.
...
части сообщений без требования полного его декодирования. Кодирование
базовых типов.
...
сообщении или инкапсуляции имеет индекс 0. Кодирование составных типов.
...
Строка кодируется как величина типа unsigned long, содержащее длину
...
байта инкапсуляции. Кодирование псевдообъектов.
