...
Прошёл месяц с тех пор как я написал первую часть статьи
по использованию DirectX в среде Delphi - 
[DirectX для начинающих](./).

У меня накопилось ещё несколько
...
Выражение вида
    ...
    if COM-объект <> nil then COM-объект := nil
...
исследовать всю Сеть в поисках ответа на такой вопрос, но, думается -
нет. Почему?
...
обозначает отсутствие цвета во всех графических режимах -
16 цветов, 256, 65535 и т.д.
...
2\. Используя файл directsetup.pas, мне не удалось экспортировать функцию
...
Скачать примеры: [samples2.zip](samples2.zip) (106 K) 

Далее: [DirectX для начинающих. Часть третья. Считывание и запись](./part3/)
