...
следующие значения:
    ...
    +-------------------------+-----------------------------------+
    | GL_LINES                | Каждая пара вершин - отдельная    |
    |                         | линия. Если число вершин нечётно, |
    |                         | то последняя игнорируется         |
    +-------------------------+-----------------------------------+
    | GL_LINE_STRIP           | Последовательность связанных      |
    ...
    +-------------------------+-----------------------------------+
    | GL_LINE_LOOP            | Аналогичен GL_LINE_STRIP,         |
    |                         | только последняя вершина          |
    |                         | соединяется отрезком с первой.    |
    +-------------------------+-----------------------------------+
    | GL_TRIANGLES            | Каждая тройка вершин - отдельный  |
    |                         | треугольник                       |
    +-------------------------+-----------------------------------+
    ...
    +-------------------------+-----------------------------------+
    | GL_POLYGON              | Рисует отдельный выпуклый         |
    |                         | многоугольник (один).             |
    +-------------------------+-----------------------------------+
...
Откровенно говоря, у меня OpenGL делает это довольно плохо, возможно
...
:::{.center}
![clip0022](clip0022.png) ![](image020.jpg)
:::
