...
В данный момент вы должны видеть нечто, подобное изображенному на
рисунке 1.

:::{.center}
Рисунок 1 - Настройка базового отчета  
...
(От переводчика - замените comctrl32.dll на более свежую).
...
Для того, чтобы сделать данный пример похожим на настоящее приложение,
...
TQRChildBand.
: Используйте объект TQRChildBand когда вам требуется
...
TQRGroup.
: Секция которая печатается при смене выражения, обычно при
...
компоненты обеспечивают действительную печать на бумагу.

TQRLabel.
: Размещает статический текст на странице.

TQRDBText.
: Эквивалент компоненты TDBText - используется для отображения
...
TQRExpr.
: Используется для отображения \'выражений\'. Обычно вы
...
TQRRichText.
: Помещает rich text (то есть многострочный текст с RTF
...
TQRDBRichText.
: Это data-aware версия TQRRichText. Используйте для печати
форматированного текста из BLOB полей.

TQRShape.
: Кузина редко используемого компонента TShape control из
...
TQRCompositeReport.
: Иногда вам требуется группировать совместно
...
TQRPreview.
: Для просмотра отчета перед печатью, все что вам нужно это
...
TQRTextFilter.
: Текстовый формат: экспортирует отчет, как простой ASCII
текст, используются пробелы для разделения полей.

TQRCSVFilter.
: CSV формат: экспортирует отчет \'Comma Separated
...
TQRHTMLFilter.
: HTML формат: экспортирует отчет в HyperText Markup
...
показывает как использовать HTML фильтр.
    ...
    quickrep1.ExportToFilter(
      TQRHTMLDocumentFilter.Create('c:\eport.txt')
    );
...
1. Создайте простой отчет, как это описано в ['Первый отчет'](#first).
...
TQRExpr, TQRSysData, TQRMemo, TQRExprMemo,
TQRRichText и TQRDBRichText - используют несколько общих свойств,
которые они наследуют:
...
и которое напечатает для значений 1234 и -1234.5 следующий вывод:

    1,234.00 и (1,234.50) соответственно.
...
Help files.
: QuickReport хелп фалы инсталлируются и интегрируются с
...
Demo applications.
: Демо-приложение Вы можете найти здесь:
...
: Стандартный репозиторий Delphi содержит шаблоны
...
Website.
: Наш сайт http://www.qusoft.com Пожалуйста приходите и поищите
