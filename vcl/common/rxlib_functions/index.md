...
транзакцию и в этом случае возвращает True,
...
CopyIconToClipboard - Копирует иконку в карман
...
FirstDayOfNextMonth - Первый день месяца

FirstDayOfPrevMonth - Первый день предыдущего месяца
...
IncTime - Увеличивает время на заданное количество часов, минут, сек и
мс, возвращая время IncYear

IsLeapYear - Проверяет является ли заданный параметром AYear год
високосным.

LastDayOfPrevMonth - Последний день предыдущего месяца

MonthsBetween - разница в месяцах
...
BrowseComputer - BrowseComputer открывает диалоговое окно, позволяющее пользователю выбрать сетевой компьютер.
...
DirExists - DirExists определяет, существует ли каталог, указанный в качестве значения параметра Name.

FileDateTime - Дата и время файла

FileLock - Функция FileLock блокирует область в открытом файле

FileUnlock - снимает блокировку

GetFileSize - Функция возвращает размер в байтах файла, заданного
параметром FileName.

GetSystemDir - Функция GetSystemDir получает путь к системному каталогу Windows.
...
AnsiProperCase - Возвращает строку, в которой первая буква каждого слова указана в верхнем регистре, а все остальные буквы — в нижнем.
...
ExtractQuotedString - удаляет символы кавычек из начала и конца строки в кавычках.
...
FindCmdLineSwitch - FindCmdLineSwitch определяет, была ли строка передана приложению в качестве аргумента командной строки.
...
IntToRoman - преобразует заданное значение в строковое представление римских чисел.
...
используя символы WordDelims как возможные разделители между словами.
...
OemToAnsiStr - OemToAnsiStr переводит строку из набора символов OEM в набор символов Windows.

QuotedString - QuotedString возвращает заданную строку в кавычках, используя предоставленный символ кавычки.
...
RomanToInt - RomanToInt преобразует данную строку в целочисленное значение.
...
выделенного функцией AllocMemo.
...
HugeDec - Уменьшает огромный указатель.

HugeInc - Увеличивает огромный указатель.
...
KillMessage - KillMessage удаляет запрошенное сообщение Msg из очереди сообщений окна.

LoadAniCursor - Функция LoadAniCursor загружает указанный ресурс анимированного курсора из исполняемого файла (.EXE или .DLL), связанного с указанным экземпляром приложения.
...
(по умолчанию crHourGlass), и увеличивает счетчик вызовов StartWait на 1.
