...
> Я пытаюсь выполнить запрос для отображения записей в dbgrid.
> Однако из-за размера таблиц и соединений выполнение запроса занимает некоторое время.
> Есть ли способ показать индикатор выполнения с таймером, который увеличивает позицию,
> но продолжает работать, пока выполняется запрос?
> Кстати, при этом доступ с BDE не используется.

**Ответ:**

Прогрессбар не будет идеальным выбором, поскольку вы не можете заранее определить,
сколько времени займет запрос, поэтому вы не знаете диапазон,
который должен охватить индикатор выполнения.

Более подходящим был бы простой вид анимации, который сообщает пользователю в основном только о том,
чтобы убедиться, что приложение не зависло.
Можно было бы сделать это во вторичном потоке,
но это пришлось бы делать с помощью простого API Windows и *никаких* вызовов Synchronize
(поскольку основной поток заблокирован в вызове BDE).

Вот пример: unit anithread;
