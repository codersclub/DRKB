...
`CombineRgn`.
Давайте посмтрим, какие значения он может принимать.
...
поступим иначе: объявим `private property fRgn`,
в `OnCreate` его инициализируем,
в `OnPaint` будем его отображать,
а в `OnDestroy` - уничтожим. Код методов представлен ниже:
...
![](6.jpg){style="width:642px; height:141px;"}
