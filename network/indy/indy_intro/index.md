...
Author: Chad Z. Hower (http://www.atozedsoftware.com)
Date: 01.01.2007
---
...
_Перевод: Анатолий Подгорецкий_
...
На форме размещен компонент IdTCPServer1 типа TIdTCPServer.
...
2.   Чтения приветствия с сервера.

3.   Для каждой строки введенной пользователем в TMemo:

4.   Посылка запроса на сервер (`WriteLn('ZipCode ' + memoInput.Lines[i];`)

5.   Чтение ответа с сервера (s := ReadLn;)

6.   Посылка команды Quit (`WriteLn('Quit');`)

7.   Разъединение (Disconnect;)
...
проверены с помощью Телнет. Для этого достаточно знать порт сервера.
Zip Code Lookup Server слушает на порту 6000.

Запустите снова Zip Code Lookup Server.
Затем откройте консоль (например, окно Dos).
Теперь введите:
