---
Title: 10. Реализация протоколов
Date: 12.12.2006
...
бывают сл		учаи, когда требуется реализовать протокол самостоятельно.
...
Простой текст (_plain text_) означает, что все команды (_commands_) и ответы
(_replies_) используют только 7-битный код ASCII. Если передача данных в
откликах (_responses_) допустима в двоичном виде, то почти все команды
...
случаев, эти данные языко-независимы (_language neutral_). Например,
...
использоваться эмуляция (_видимо симуляция, судя по названию главы_)
...
откликов. Если код не найден, то отклик не возвращается (_но если судить
по примеру, это не так, возвращает пустой отклик - строка с точкой_).
Код ответа "200 Ok".
...
Почтовый сервер отвечает аналогичным образом:
