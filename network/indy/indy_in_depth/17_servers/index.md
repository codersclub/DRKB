...
![taskman.png](taskman.png){.center}

Моя система имеет 513 созданных потоков.

![taskman2.png](taskman2.png){.center}

Вы видите, что даже при 513 потоках процессор нагружен только на 1%. Сильно
...
потока, они выполняются внутри потока. _Это очень важная деталь. Будьте
уверены, что вы понимаете эту деталь до начала обработки._
...
команды, которые не совместимы (_от корректора: труднореализуемые??? Не
могу себе представить что он имел  в виду, потому что conversational это
словоохотливый, разговорчивый_), реализация обработчиков команд не
...
инструментов диалога. После это в списке появится обработчик команд.

![edit2.png](edit2.png){.center}
...
![inspector.png](inspector.png){.center}
...
_Примечание: используйте свойство ASender.Reply.Text.Text. Указание слова
...
проинициализировано и запись в него вызовет перекрытия текста._
