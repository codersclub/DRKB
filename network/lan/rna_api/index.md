...
`m_flags` - самое интересное для нас поле. Если младший его бит равен 1,
RNA сервер для данного устройства включён, 0 - выключен.

`m_szProtocol` - чаще всего это поле просто забито нулями, но иногда в
...
`m_szComment` - комментарий. Тот самый, что можно ввести в диалоге
Dial-Up Server, в поле Comment.

`m_accessNumber` - как минимум означает тип контроля за разграничением
доступа к серверу. Если равен 0 - значит используется "share level
access control". При этом проверяется только пароль.
Если не равен нулю - значит производится проверка имени пользователя и пароля,
то есть "user level access control". Что конкретно при этом означает сама цифра,
...
включён/выключен определяется младшим битом в поле m_flags структуры
...
Возвращаемое значение. ???
    ...
    void __stdcall SuprvEnumAccessInfo( char* _pStringsBuffer,
          int* _pStringsBufferSize, unsigned int* _pnStrings );
...
Возвращаемое значение. Нет.
    ...
    int __stdcall SuprvGetAccessInfo( char* _pAccessInfoString,
...
успешное завершение функции.
    ...
    int __stdcall RnaSMMInfoDialog( HWND _hwnd, DWORD _dwUnused,
...
функции просто помещает в этот параметр указатель на поле m_szProtocol
структуры ACCESSINFO.
