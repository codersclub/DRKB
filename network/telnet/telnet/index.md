...
Title: Telnet
Date: 01.01.2007
Source: <https://www.delphirus.com.ru>
---
...
- `unknown host` -  Имя или адрес были набраны неправильно
- `connection refused` - Удаленный компьютер функционирует с ошибками
...
**Некоторые примеры и адреса**
    ...
    function netsharegetinfo(const pszserver: pchar; const psznetname: pchar;
      ...
      external 'svrapi.dll' name 'netsharegetinfo';
    ...
    begin
      ...
      if ansiuppercase(compname) = ansiuppercase(edcomputernetname.text) then exit;
...
- martini.eecs.umich.edu 3000 или 141.212.99.9 3000  
информация о городах США, население, географическое положение и др.
...
**archie (поиск нужных файлов по всем анонимным ftp серверам)**
