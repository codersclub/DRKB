...
Date: 01.01.2007
Source: http://www.interface.ru/fset.asp?Url=/borland/aspart.htm
ID: 03426
---
...
признакам ASP сервер напоминает CGI приложения и ISAPI/NSAPI dll
(далее - WEB сервер приложения). Идеология выполнения методов в ASP сервере и
...
\*Write для этих свойств определим чтение и возврат данных из описанных
...
:::{.center}
![](clip0322.gif)  
Рисунок 6. Чтение и показ ресурсов в HTML документе
:::
...
В заключении следует рассмотреть out-of-process ASP сервера.
...
Все, что было сказано выше для in-process сервера применимо и
...
необходимо выполнить следующий VB скрипт:
