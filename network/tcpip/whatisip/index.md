...
- `Адрес источника (SOURCE IP ADDRESS)` и `Адрес назначения (DESTINATION
...
В большинстве типов локальных и глобальных сетей определяется такое
...
Особенно часто приемом маршрутизации по умолчанию пользуются конечные
...
2) Будем считать, что компьютер cit.dol.ru имеет файл HOSTS, а в нем есть
строка
...
4) Компьютер cit.dol.ru начинает формировать кадр Ethernet для отправки
...
5) В результате компьютер cit.dol.ru отправляет по локальной сети кадр
