...
Пусть `m = p1e1 * p2e2 * ... * pkek` - представление целого числа m
в виде произведения степеней простых. Число
...
Покажем, что число 561 является кармайкловым.
...
а затем проверить, не является ли m степенью простого
...
Если xt =/= 1 (mod m), то m не удовлетворяет малой теореме Ферма и,
следовательно, не является простым.
Если же последовательность (1) содержит фрагмент `..., a, 1, ...`,
где a =/= +-1 (mod m), то имеем
