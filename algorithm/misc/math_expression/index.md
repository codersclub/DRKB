...
[https://www.rxlib.ru/Downl/Downl.htm](https://www.rxlib.ru/Downl/Downl.htm)
...
Каждый шаг выделен в отдельную процедуру. Это позволяет быстрее считать
...
Отличная реализация есть в бесплатной библиотеке для дельфи JVCL. Помимо
...
- sinc (x) sinh (x) cosh (x) tanh (x) coth (x) sin (x) cos (x) tan (x) cot (x) sqrt (x) sqr (x)
- arcsinh (x) arccosh (x) arctanh (x) arccoth (x) arcsin (x) arccos (x) arctan (x) arccot (x)
